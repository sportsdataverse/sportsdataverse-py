...
- **`generate.py --docs`** renders the full reference docs tree from the same
  endpoint/loader/parameter metadata that drives the wrappers, into a tracked
  staging mirror (`tools/codegen/_generated_docs/`, parallel to `_generated/`).
  64 files: per-league `index.md` + `_category_.json`, a per-API reference page
  for every ESPN API (`site`/`web`/`core`) and native flat family
  (`nhl_api_web`/`nhl_edge`/`nhl_stats_rest`/`nhl_records`/`mlb_api`), a
  `reference/loaders.md` per loader league, a shared `reference/parameters.md`,
  and (when a `packages.json` snapshot exists) `packages.mdx`.
...
- **Drift gate**: `--check` now also fails on stale/orphan generated docs, and the
  default `build` writes them; `tools/codegen/fetch_packages.py` snapshots the SDV
  package list for the packages page (network tool; the gate stays offline by
  omitting the page when no snapshot is committed). New offline tests
  `tests/codegen/test_docs.py` + `test_doc_parity.py` assert the 8-section contract
  across every league x API and that the staging tree is current.
- Wiring the generated tree into the live Docusaurus site (autogenerated sidebar,
  data-driven navbar, retiring `create_docs.sh`/`Sphinx-docs`) and the example
  notebooks remain follow-ups (need a Node build + live endpoints).
