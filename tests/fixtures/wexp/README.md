...
| `cfb_pbp_drive_sample.parquet` | 324 | `espn_cfb_pbp` release (season 2019, captured 2026-08-07) | Two full games (401110720, 401112224), 9 pruned play columns (`EPA`, `EP_start`/`EP_end`, `drive.id`, `pos_team`/`def_pos_team` ids, `game_play_number`). Feeds the drive-EP response extraction tests (`cfb_drive_ep_responses`): drive delta = sum(EPA) — raw `EP_end` flips perspective on drive-ending plays, the sign bug the corrected extractor guards. |
