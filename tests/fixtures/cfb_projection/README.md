...
| `recruits_2014_2023.parquet` | ~39.6k | **247Sports RDB recruit feed** — `sports247_recruits(sport_key=1, year=Y)`, classes 2014-2023, paginated (page_size=200 + per-page retry; the RDB times out under rapid paging). Team = **signed** institution (falls back to committed) — the RDB's `committed_institution` drifts with decommits/transfers. | `season, team_id, team, recruit_id, stars, grade, position` |
| `teams_2023.parquet` | ~1.8k | **ESPN team info** — `load_cfb_team_info(2023)`: school/mascot/espn `team_id`/`classification` — the offline name→id map (school-only vs school+mascot key spaces). | `team_id, school, mascot, classification` |
| `draft_2017_2024.parquet` | ~2.1k | **nflverse draft picks** — `load_nfl_draft_picks()` filtered to 2017-2024 (the ESPN season-draft endpoint 404s). College is PFR-style; `player_id` = ESPN `cfb_player_id` (recent drafts only). | `draft_year, round, pick, player_name, college, player_id, position` |
| `results_2004_2025.parquet` | ~42.4k | Captured **2026-09-17** by `capture_oracle.py gate_2025`: `load_cfb_schedule(range(2004, 2026))`, **completed** games only (canceled/postponed games carry 0-0 scores, 164 of them 2016-2024, which the 2016-2023 capture's points-not-null filter admits), same columns as `results_2016_2023` | `game_id, season, week, home_team_id, away_team_id, home_score, away_score, neutral_site` |
| `returning_2005_2025.parquet` | ~4.3k | Captured **2026-09-17** by `capture_oracle.py gate_2025`: `cfb_returning_production(2005..2025)` (defense: pbp splash for 2005-2014, play participants from 2015) + per-season `is_fbs` from `load_cfb_teams`. Computed from the `cfbfastR-cfb-data` release copies of that date (pbp republished 2026-09-16). `overall_returning` / `overall_basis` were re-derived under the shipped 0.49/0.51 weights; the gates recompute them from the unit columns anyway. Read by the returning-production gates and `fit_returning_weights.py`; the recruiting backtest still reads `returning_2017_2023` | `season, team_id, off_returning, def_returning, overall_returning, n_returning, def_basis, overall_basis, is_estimated, is_fbs` |
