...
def retrosplits_game_logs_player(first_season: int, last_season=None) -> pd.DataFrame():
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            game_log_url = (
                f"https://raw.githubusercontent.com/chadwickbureau/retrosplits/master/daybyday/playing-{season}.csv"
            )
...
def retrosplits_game_logs_team(first_season: int, last_season=None) -> pd.DataFrame():
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            game_log_url = (
                f"https://raw.githubusercontent.com/chadwickbureau/retrosplits/master/daybyday/team-{season}.csv"
            )
            game_log_df = pd.read_csv(game_log_url, sep=",")
        except Exception as e:
            game_log_df = pd.DataFrame()
...
def retrosplits_player_batting_by_platoon(first_season: int, last_season=None) -> pd.DataFrame():
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        print(season)
        game_log_url = (
            f"https://raw.githubusercontent.com/chadwickbureau/retrosplits/master/splits/batting-platoon-{season}.csv"
        )
        try:
            game_log_df = pd.read_csv(game_log_url, sep=",")
        except Exception as e:
...
def retrosplits_player_head_to_head_stats(first_season: int, last_season=None) -> pd.DataFrame():
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        print(season)
        game_log_url = (
            f"https://raw.githubusercontent.com/chadwickbureau/retrosplits/master/splits/headtohead-{season}.csv"
        )
        try:
            game_log_df = pd.read_csv(game_log_url, sep=",")
        except Exception as e:
...
def retrosplits_player_pitching_by_platoon(first_season: int, last_season=None) -> pd.DataFrame():
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        print(season)
        game_log_url = (
            f"https://raw.githubusercontent.com/chadwickbureau/retrosplits/master/splits/pitching-platoon-{season}.csv"
        )
        try:
            game_log_df = pd.read_csv(game_log_url, sep=",")
        except Exception as e:
