...
- feat(release): new `sportsdataverse.release` module — a Python port of the
  ...
  `sportsdataverse_save()` (writes csv / csv.gz / parquet with
  `sportsdataverse_type` + `sportsdataverse_timestamp` parquet metadata and
  ...
  (the multi-file form silently drops large assets), R-only `rds`/`qs`
  formats raise `ValueError` (Python default `("csv", "parquet")`), unpadded
  `size_string`, no `.token` argument (gh CLI auth; `GH_TOKEN` falls back to
  `GITHUB_PAT`).
