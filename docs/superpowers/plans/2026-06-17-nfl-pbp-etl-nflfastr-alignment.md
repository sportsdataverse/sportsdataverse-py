...
- [NFL PBP ETL ↔ nflfastR Alignment — Implementation Plan](#nfl-pbp-etl-%E2%86%94-nflfastr-alignment--implementation-plan)
  - [Global Constraints](#global-constraints)
    - [T1: Pin the shared column contract + centralize constants](#t1-pin-the-shared-column-contract--centralize-constants)
    - [T2: `calculate_epa(df)` in ep_wp.py (lift the EPA derivation)](#t2-calculate_epadf-in-ep_wppy-lift-the-epa-derivation)
    - [T3: `calculate_wpa(df)` in ep_wp.py (lift WP-end/start + WPA)](#t3-calculate_wpadf-in-ep_wppy-lift-wp-endstart--wpa)
    - [T4: `enrich_nfl_pbp(frame)` orchestrator](#t4-enrich_nfl_pbpframe-orchestrator)
    - [T5: Efficiency in the scorers](#t5-efficiency-in-the-scorers)
    - [T6: Gut `NFLPlayProcess` model steps → delegate to ep_wp](#t6-gut-nflplayprocess-model-steps-%E2%86%92-delegate-to-ep_wp)
    - [T7: Add missing nflfastR construction methods (fixed_drives + series)](#t7-add-missing-nflfastr-construction-methods-fixed_drives--series)
    - [T8: Per-game cache + `build_nfl_season` compile helper](#t8-per-game-cache--build_nfl_season-compile-helper)
    - [T9: Pin models + docs + final regression](#t9-pin-models--docs--final-regression)
...
### T1: Pin the shared column contract + centralize constants
...
### T2: `calculate_epa(df)` in ep_wp.py (lift the EPA derivation)
...
### T3: `calculate_wpa(df)` in ep_wp.py (lift WP-end/start + WPA)
...
### T4: `enrich_nfl_pbp(frame)` orchestrator
...
### T6: Gut `NFLPlayProcess` model steps → delegate to ep_wp
...
### T7: Add missing nflfastR construction methods (fixed_drives + series)
...
### T8: Per-game cache + `build_nfl_season` compile helper
...
### T9: Pin models + docs + final regression
