...
| `duration` | Int64 | Penalty duration in minutes. |
...
| `eventId` | Int64 | Event identifier within the game. |
| `strength` | String | Strength label (Even, Power Play, Shorthanded). |
| `playerId` | Int64 | Player identifier involved in the event. |
...
| `away` | Int64 | Away team shots in the period. |
| `game_id` | Int64 | Unique game identifier. |
| `season` | Int64 | Season year (echoed from arg). |
| `game_date` | String | Game date. |
| `sequence` | Int64 | Sequence order of the season row. |
| `playerId` | Int64 | Player identifier involved in the event. |
...
| `period` | Int64 | Period number. |
| `period_type` | String | Period type (REG/OT/SO). |
| `max_regulation_periods` | Int64 | Number of regulation periods the game format defines before overtime, constant at 3 for every row in the published seasons. |
| `ot_periods` | Int64 | Overtime period ordinal taken from the NHL period descriptor, populated only from the second overtime onward so period 5 rows carry 2 and all other rows are null. |
| `away` | Int64 | Away team shots in the period. |
| `home` | Int64 | Whether the player's team was home. |
| `game_id` | Int64 | Unique game identifier. |
| `season` | Int64 | Season year (echoed from arg). |
