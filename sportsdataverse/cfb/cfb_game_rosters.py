...
def espn_cfb_game_rosters(game_id: int, raw=False, return_as_pandas=False, **kwargs) -> pl.DataFrame:
    ...
    summary_url = "https://sports.core.api.espn.com/v2/sports/football/leagues/college-football/events/{x}/competitions/{x}/competitors".format(
        x=game_id,
    )
    summary_resp = download(summary_url, **kwargs)
    summary = summary_resp.json()
    items = helper_cfb_game_items(summary)
    team_rosters = helper_cfb_roster_items(items=items, summary_url=summary_url, **kwargs)
    team_rosters = team_rosters.join(items[["team_id", "order", "home_away", "winner"]], how="left", on="team_id")
    teams_df = helper_cfb_team_items(items=items, **kwargs)
    teams_rosters = team_rosters.join(teams_df, how="left", on="team_id")
    athletes = helper_cfb_athlete_items(teams_rosters=team_rosters, **kwargs)
...
def helper_cfb_game_items(summary):
    ...
    Example:
        Used internally by :func:`espn_cfb_game_rosters`::
...
def helper_cfb_team_items(items, **kwargs):
    ...
    Args:
        items (pl.DataFrame): Output of :func:`helper_cfb_game_items`.
...
def helper_cfb_roster_items(items, summary_url, **kwargs):
...
def helper_cfb_athlete_items(teams_rosters, **kwargs):
    """Internal helper that resolves each athlete ``$ref`` in a team-rosters frame
    to the canonical athlete detail row.

    Args:
        teams_rosters (pl.DataFrame): Output of :func:`helper_cfb_roster_items`
