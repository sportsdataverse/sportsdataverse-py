...
def load_cfb_pbp(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            CFB_BASE_URL.format(season=i), engine="auto", columns=None
        )
...
def load_cfb_schedule(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            CFB_TEAM_SCHEDULE_URL.format(season=i), engine="auto", columns=None
        )
        # data = data.append(i_data)
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
...
def load_cfb_rosters(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            CFB_ROSTER_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_cfb_team_info(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        try:
            i_data = pd.read_parquet(
                CFB_TEAM_INFO_URL.format(season=i), engine="auto", columns=None
            )
        except Exception:
