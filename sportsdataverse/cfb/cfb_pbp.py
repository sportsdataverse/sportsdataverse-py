...
class CFBPlayProcess(object):
    ...
    def __helper_cfb_pbp_features(self, pbp_txt,
        ...
        for key in pbp_txt.get("drives").keys():
            ...
            pbp_txt["plays"] = pl.concat(
                [pbp_txt["plays"], pl.from_pandas(prev_drives)], how='vertical'
            )
        ...
        pbp_txt["plays"] = pbp_txt["plays"].with_columns(
            ...
            status_type_completed = pbp_txt.get("header").get("competitions")[0].get("status").get("type").get("completed"),
            homeTeamId = pl.lit(homeTeamId),
            awayTeamId = pl.lit(awayTeamId),
            homeTeamName = pl.lit(str(homeTeamName)),
            awayTeamName = pl.lit(str(awayTeamName)),
            homeTeamMascot = pl.lit(str(homeTeamMascot)),
            awayTeamMascot = pl.lit(str(awayTeamMascot)),
            homeTeamAbbrev = pl.lit(str(homeTeamAbbrev)),
            awayTeamAbbrev = pl.lit(str(awayTeamAbbrev)),
            homeTeamNameAlt = pl.lit(str(homeTeamNameAlt)),
            awayTeamNameAlt = pl.lit(str(awayTeamNameAlt)),
            homeTeamSpread = pl.when(homeFavorite == True)
                .then(abs(gameSpread))
                .otherwise(-1 * abs(gameSpread)),
            gameSpread = pl.lit(gameSpread),
            gameSpreadAvailable = pl.lit(gameSpreadAvailable),
            overUnder = pl.lit(float(overUnder)),
        ...
        pbp_txt["plays"] = pbp_txt["plays"].with_columns(
            ...
            text_dupe = pl.when((pl.col("start.team.id") == pl.col("lead_start_team"))
                ...
                .then(pl.lit(True))
                .when((pl.col("start.team.id") == pl.col("lead_start_team"))
                    .and_(pl.col("start.down") == pl.col("lead_start_down"))
                    .and_(pl.col("start.yardsToEndzone") == pl.col("lead_start_yardsToEndzone"))
                    .and_(pl.col("start.distance") == pl.col("lead_start_distance"))
        ...
        pbp_txt["plays"] = pbp_txt["plays"].with_columns(
            pl.col("start.team.id").fill_null(strategy="forward")
                .fill_null(strategy="backward").cast(pl.Int32)
        ).with_columns(
            pl.col("end.team.id").fill_null(value=pl.col("start.team.id")).cast(pl.Int32)
        ).with_columns(
            ...
            pl.when(pl.col("type.text").is_in(kickoff_vec)
                .and_(pl.col("start.team.id") == homeTeamId))
                .then(pl.col("awayTeamId"))
                .when(pl.col("type.text").is_in(kickoff_vec)
                    .and_(pl.col("start.team.id") == awayTeamId))
                .then(pl.col("homeTeamId"))
                .otherwise(pl.col("start.team.id"))
                .alias("start.pos_team.id")
        ).with_columns(
            pl.when(pl.col("start.pos_team.id") == homeTeamId)
                .then(awayTeamId)
                .otherwise(homeTeamId)
                .alias("start.def_pos_team.id"),
            pl.when(pl.col("end.team.id") == homeTeamId)
                .then(awayTeamId)
                .otherwise(homeTeamId)
                .alias("end.def_pos_team.id"),
            pl.col("end.team.id")
                .alias("end.pos_team.id")
        ).with_columns(
            pl.when(pl.col("start.pos_team.id") == homeTeamId)
            ...
            pl.when(pl.col("end.pos_team.id") == homeTeamId)
                .then(pl.col("homeTeamName"))
                .otherwise(pl.col("awayTeamName"))
        ...
        pbp_txt["plays"] = pbp_txt["plays"].with_columns(
            pl.when((
                (pbp_txt["timeouts"][homeTeamId]["1"] <= pl.col("id"))
                    .and_(pl.col("period.number") <= 2,
                        len(pbp_txt["timeouts"][homeTeamId]["1"]) == 1)
                ).or_(
                (pbp_txt["timeouts"][homeTeamId]["2"] <= pl.col("id"))
                    .and_(pl.col("period.number") > 2,
                        len(pbp_txt["timeouts"][homeTeamId]["2"]) == 1)
            ))
            .then(2)
            .when((
                (pbp_txt["timeouts"][homeTeamId]["1"] <= pl.col("id"))
                    .and_(pl.col("period.number") <= 2,
                        len(pbp_txt["timeouts"][homeTeamId]["1"]) == 2)
                ).or_(
                (pbp_txt["timeouts"][homeTeamId]["2"] <= pl.col("id"))
                    .and_(pl.col("period.number") > 2,
                        len(pbp_txt["timeouts"][homeTeamId]["2"]) == 2)
            ))
            .then(1)
            .when((
                (pbp_txt["timeouts"][homeTeamId]["1"] <= pl.col("id"))
                    .and_(pl.col("period.number") <= 2,
                        len(pbp_txt["timeouts"][homeTeamId]["1"]) == 3)
                ).or_(
                (pbp_txt["timeouts"][homeTeamId]["2"] <= pl.col("id"))
                    .and_(pl.col("period.number") > 2,
                        len(pbp_txt["timeouts"][homeTeamId]["2"]) == 3)
            ))
            .then(0)
            .otherwise(3)
            .alias("end.homeTeamTimeouts"),
            pl.when((
                (pbp_txt["timeouts"][awayTeamId]["1"] <= pl.col("id"))
                    .and_(pl.col("period.number") <= 2,
                        len(pbp_txt["timeouts"][awayTeamId]["1"]) == 1)
                ).or_(
                (pbp_txt["timeouts"][awayTeamId]["2"] <= pl.col("id"))
                    .and_(pl.col("period.number") > 2,
                        len(pbp_txt["timeouts"][awayTeamId]["2"]) == 1)
            ))
            .then(2)
            .when((
                (pbp_txt["timeouts"][awayTeamId]["1"] <= pl.col("id"))
                    .and_(pl.col("period.number") <= 2,
                        len(pbp_txt["timeouts"][awayTeamId]["1"]) == 2)
                ).or_(
                (pbp_txt["timeouts"][awayTeamId]["2"] <= pl.col("id"))
                    .and_(pl.col("period.number") > 2,
                        len(pbp_txt["timeouts"][awayTeamId]["2"]) == 2)
            ))
            .then(1)
            .when((
                (pbp_txt["timeouts"][awayTeamId]["1"] <= pl.col("id"))
                    .and_(pl.col("period.number") <= 2,
                        len(pbp_txt["timeouts"][awayTeamId]["1"]) == 3)
                ).or_(
                (pbp_txt["timeouts"][awayTeamId]["2"] <= pl.col("id"))
                    .and_(pl.col("period.number") > 2,
                        len(pbp_txt["timeouts"][awayTeamId]["2"]) == 3)
            ))
            .then(0)
            .otherwise(3)
            .alias("end.awayTeamTimeouts")
        ).with_columns(
            pl.col("end.homeTeamTimeouts").shift_and_fill(periods = 1, fill_value = 3)
                .alias("start.homeTeamTimeouts"),
            pl.col("end.awayTeamTimeouts").shift_and_fill(periods = 1, fill_value = 3)
                .alias("start.awayTeamTimeouts"),
            pl.col("start.TimeSecsRem").shift(periods = 1)
                .alias("end.TimeSecsRem"),
            pl.col("start.adj_TimeSecsRem").shift(periods = 1)
            ...
            pl.when((pl.col("game_play_number") == 1)
                .and_(pl.col("type.text").is_in(kickoff_vec),
            ...
            pl.when((pl.col("type.text") == "Penalty")
                .and_(pl.col("text").str.contains(r"(?i)declined")))
                .then(pl.col("start.yard"))
            ...
            pl.when((pl.col("start.distance") == 0)
                .and_(pl.col("start.downDistanceText").str.contains(r"(?i)goal")))
                .then(pl.col("start.yardsToEndzone"))
        ...
        pbp_txt["plays"] = pbp_txt["plays"].with_columns(
            ...
            pl.when(pl.col("type.text").str.to_lowercase().str.contains("(?i)extra point")
            ...
            pl.when(pl.col("type.text").str.to_lowercase().str.contains("(?i)field goal")
                .and_(pl.col("type.text").str.to_lowercase().str.contains("(?i)blocked")))
                .then("Extra Point Missed")
                .otherwise(pl.col("type.text"))
                .alias("type.text")
    ...
    def __add_downs_data(self, play_df):
        ...
        play_df = play_df.filter(
                pl.col("type.text").str.contains(
                    "(?i)end of|(?i)coin toss|(?i)end period|(?i)wins toss"
                ) == False
        )
    ...
    def __add_play_type_flags(self, play_df):
        ...
        play_df = play_df.with_columns(
            ...
            .then(True)
            .when((pl.col("text").str.contains("(?i)fumble"))
                .and_(pl.col("type.text") == "Rush",
                    pl.col("start.pos_team.id") != pl.col("end.pos_team.id")))
            .then(True)
            .when((pl.col("text").str.contains("(?i)fumble"))
                .and_(pl.col("type.text") == "Sack",
                    pl.col("start.pos_team.id") != pl.col("end.pos_team.id")))
            .then(True)
            .otherwise(False),
            forced_fumble = pl.when(pl.col("text").str.contains("(?i)forced by"))
                .then(True)
                .otherwise(False),
            # --- Kicks----
            kickoff_play = pl.col("type.text").is_in(kickoff_vec),
        ).with_columns(
            kickoff_tb = pl.when((pl.col("text").str.contains("(?i)touchback"))
                .and_(pl.col("kickoff_play") == True))
                .then(True)
                .when((pl.col("text").str.contains("(?i)kickoff$"))
                    .and_(pl.col("kickoff_play") == True))
                .then(True)
                .otherwise(False),
            kickoff_onside = pl.when((pl.col("text").str.contains("(?i)on-side|(?i)onside|(?i)on side"))
                .and_(pl.col("kickoff_play") == True))
                .then(True)
                .otherwise(False),
            kickoff_oob = pl.when((pl.col("text").str.contains("(?i)out-of-bounds|(?i)out of bounds"))
                .and_(pl.col("kickoff_play") == True))
                .then(True)
                .otherwise(False),
            kickoff_fair_catch = pl.when((pl.col("text").str.contains("(?i)fair catch|(?i)fair caught"))
                .and_(pl.col("kickoff_play") == True))
                .then(True)
                .otherwise(False),
            kickoff_downed = pl.when((pl.col("text").str.contains("(?i)downed"))
                .and_(pl.col("kickoff_play") == True))
                .then(True)
                .otherwise(False),
            kick_play = pl.col("text").str.contains("(?i)kick|(?i)kickoff"),
            kickoff_safety = pl.when((pl.col("text").str.contains("(?i)kickoff"))
            ...
            punt_tb = pl.when((pl.col("text").str.contains("(?i)touchback"))
                .and_(pl.col("punt") == True))
                .then(True)
                .otherwise(False),
            punt_oob = pl.when((pl.col("text").str.contains("(?i)out-of-bounds|(?i)out of bounds"))
                .and_(pl.col("punt") == True))
                .then(True)
                .otherwise(False),
            punt_fair_catch = pl.when((pl.col("text").str.contains("(?i)fair catch|(?i)fair caught"))
                .and_(pl.col("punt") == True))
                .then(True)
                .otherwise(False),
            punt_downed = pl.when((pl.col("text").str.contains("(?i)downed"))
                .and_(pl.col("punt") == True))
                .then(True)
                .otherwise(False),
            punt_safety = pl.when((pl.col("text").str.contains("(?i)punt"))
                .and_(pl.col("safety") == True))
                .then(True)
                .otherwise(False),
            punt_blocked = pl.when((pl.col("text").str.contains("(?i)blocked"))
                .and_(pl.col("punt") == True))
                .then(True)
                .otherwise(False),
            penalty_safety = pl.when((pl.col("type.text").is_in(["Penalty"]))
                .and_(pl.col("safety") == True))
                .then(True)
                .otherwise(False),
    ...
    def __add_rush_pass_flags(self, play_df):
        ...
        play_df = play_df.with_columns(
            # --- Pass/Rush----
            pl.when((pl.col("type.text") == "Rush")
                    .or_(pl.col("type.text") == "Rushing Touchdown")
                    .or_((pl.col("type.text").is_in(["Safety", "Fumble Recovery (Opponent)",
                    ...
                    .otherwise(False)
                    .alias("rush"),
            pl.when((pl.col("type.text").is_in(["Pass Reception", "Pass Completion",
                                                        "Passing Touchdown", "Sack", "Pass",
                                                        "Interception", "Pass Interception Return",
                                                        "Interception Return Touchdown",
                                                        "Pass Incompletion", "Sack Touchdown",
                                                        "Interception Return"]))
                        .or_((pl.col("type.text") == "Safety")
                            .and_(pl.col("text").str.contains("sacked")))
                        .or_((pl.col("type.text") == "Safety")
                            .and_(pl.col("text").str.contains("pass complete")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Own)")
                            .and_(pl.col("text").str.contains(r"pass complete|pass incomplete|pass intercepted")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Own)")
                            .and_(pl.col("text").str.contains("sacked")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Own) Touchdown")
                            .and_(pl.col("text").str.contains(r"pass complete|pass incomplete|pass intercepted")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Own) Touchdown")
                            .and_(pl.col("text").str.contains("sacked")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Opponent)")
                            .and_(pl.col("text").str.contains(r"pass complete|pass incomplete|pass intercepted")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Opponent)")
                            .and_(pl.col("text").str.contains("sacked")))
                        .or_((pl.col("type.text") == "Fumble Recovery (Opponent) Touchdown")
                            .and_(pl.col("text").str.contains(r"pass complete|pass incomplete")))
                        .or_((pl.col("type.text") == "Fumble Return Touchdown")
                            .and_(pl.col("text").str.contains(r"pass complete|pass incomplete")))
                        .or_((pl.col("type.text") == "Fumble Return Touchdown")
                            .and_(pl.col("text").str.contains("sacked"))))
                    .then(True)
            ...
            # --- Sacks----
            sack_vec = pl.when((pl.col("type.text").is_in(["Sack", "Sack Touchdown"]))
                    .or_((pl.col("type.text").is_in([
                                "Fumble Recovery (Own)",
                                "Fumble Recovery (Own) Touchdown",
    ...
    def __add_team_score_variables(self, play_df):
        ...
        play_df = play_df.with_columns(
            ...
            pl.when(pl.col("game_play_number") == 1).then(0).otherwise(pl.col("lag_homeScore")).alias("start.homeScore"),
            pl.when(pl.col("game_play_number") == 1).then(0).otherwise(pl.col("lag_awayScore")).alias("start.awayScore"),
            ...
            pl.when(pl.col("lag_pos_team").is_null()).then(pl.col("pos_team")).otherwise(pl.col("lag_pos_team")).alias("lag_pos_team"),
            ...
            pl.when(pl.col("lag_pos_score_diff").is_null()).then(0).otherwise(pl.col("lag_pos_score_diff")).alias("lag_pos_score_diff"),
        ).with_columns(
            pl.when(pl.col("lag_pos_team") == pl.col("pos_team"))
                ...
                .alias("pos_score_pts"),
            pl.when((pl.col("kickoff_play") == True)
                .and_(pl.col("lag_pos_team") == pl.col("pos_team")))
                .then(pl.col("lag_pos_score_diff"))
                .when((pl.col("kickoff_play") == True)
                    .or_(pl.col("lag_pos_team") != pl.col("pos_team")))
    ...
    def __add_new_play_types(self, play_df):
        ...
        play_df = play_df.with_columns(
            ...
            pl.when((pl.col("fumble_vec") == True)
            ...
            pl.when((pl.col("type.text").is_in(["Blocked Field Goal"]))
                .and_(pl.col("text").str.contains("(?i)for a TD")))
                .then("Blocked Field Goal Touchdown")
                .otherwise(pl.col("type.text"))
                .alias("type.text"),
        ).with_columns(
            pl.when((pl.col("type.text").is_in(["Blocked Punt"]))
                .and_(pl.col("text").str.contains("(?i)for a TD")))
            ...
            pl.col("type.text").str.replace(r"(?i)Touchdown Touchdown", "Touchdown")
                .alias("type.text")
        ).with_columns(
            ...
            pl.when((pl.col("text").str.contains("(?i)sacked"))
            ...
            pl.when((pl.col("type.text") == "Pass")
                .and_(pl.col("text").str.contains("(?i)pass complete")))
            ...
            pl.when((pl.col("type.text") == "Pass")
                .and_(pl.col("text").str.contains("(?i)pass incomplete")))
            ...
            pl.when((pl.col("type.text") == "Pass")
                .and_(pl.col("text").str.contains("(?i)pass intercepted")))
            ...
            pl.when((pl.col("type.text") == "Passing Touchdown")
                .and_(pl.col("text").str.contains("(?i)pass intercepted for a TD")))
                .then("Interception Return Touchdown")
                .otherwise(pl.col("type.text"))
                .alias("type.text"),
        ).with_columns(
            ...
            pl.when((pl.col("type.text") == "Kickoff Touchdown")
                .and_(pl.col("fumble_vec") == False))
                .then("Kickoff Return Touchdown")
                .otherwise(pl.col("type.text"))
                .alias("type.text"),
        ).with_columns(
            pl.when((pl.col("type.text") == "Kickoff")
                    .and_(pl.col("td_play") == True)
                    .and_(pl.col("fumble_vec") == False))
                .then("Kickoff Return Touchdown")
                .when((pl.col("type.text") == "Kickoff")
                    .and_(pl.col("text").str.contains("(?i)for a TD"))
                    .and_(pl.col("fumble_vec") == False))
                .then("Kickoff Return Touchdown")
                .otherwise(pl.col("type.text"))
            ...
            pl.when((pl.col("type.text").is_in(["Kickoff", "Kickoff Return (Offense)"]))
            ...
            pl.when((pl.col("type.text") == "Punt Touchdown")
                    .and_(pl.col("fumble_vec") == False)
                    .and_(pl.col("change_of_poss") == 1))
                .then("Punt Return Touchdown")
                .when((pl.col("type.text") == "Punt")
                    .and_(pl.col("text").str.contains("(?i)for a TD"))
                    .and_(pl.col("change_of_poss") == 1))
                .then("Punt Return Touchdown")
                .otherwise(pl.col("type.text"))
            ...
            pl.when((pl.col("type.text").is_in(["Fumble Return Touchdown"]))
                    .and_((pl.col("pass") == True)
                        .or_(pl.col("rush") == True)))
                .then("Fumble Recovery (Opponent) Touchdown")
                .otherwise(pl.col("type.text"))
                .alias("type.text"),
        ).with_columns(
            # --- Safeties (kickoff, punt, penalty) ----
            pl.when((pl.col("type.text").is_in(["Pass Reception", "Rush", "Rushing Touchdown"]))
                    .and_((pl.col("pass") == True)
                            .or_(pl.col("rush") == True))
                    .and_(pl.col("safety") == True))
                .then("Safety")
            ...
            pl.when((pl.col("type.text") == "Extra Point Good")
                    .and_(pl.col("text").str.contains("(?i)Two-Point")))
                .then("Two-Point Conversion Good")
                .otherwise(pl.col("type.text"))
                .alias("type.text"),
        ).with_columns(
            pl.when((pl.col("type.text") == "Extra Point Missed")
                    .and_(pl.col("text").str.contains("(?i)Two-Point")))
    ...
    def __setup_penalty_data(self, play_df):
        ...
        play_df = play_df.with_columns(
            ...
            penalty_flag = pl.when((pl.col("type.text") == "Penalty")
                    .or_(pl.col("text").str.contains("(?i)penalty")))
                .then(True)
                .otherwise(False),
            # -- T/F flag conditions penalty_declined
            penalty_declined = pl.when((pl.col("type.text") == "Penalty")
                    .and_(pl.col("text").str.contains("(?i)declined")))
                .then(True)
                .otherwise(False),
            # -- T/F flag conditions penalty_no_play
            penalty_no_play = pl.when((pl.col("type.text") == "Penalty")
                    .and_(pl.col("text").str.contains("(?i)no play")))
                .then(True)
                .otherwise(False),

            # -- T/F flag conditions penalty_offset
            penalty_offset = pl.when((pl.col("type.text") == "Penalty")
                    .and_(pl.col("text").str.contains("(?i)off-setting")))
                .then(True)
                .when((pl.col("text").str.contains("(?i)penalty"))
                    .and_(pl.col("text").str.contains("(?i)off-setting")))
                .then(True)
                .otherwise(False),
            # -- T/F flag conditions penalty_1st_conv
            penalty_1st_conv = pl.when((pl.col("type.text") == "Penalty")
                    .and_(pl.col("text").str.contains("(?i)1st down")))
                .then(True)
                .when((pl.col("text").str.contains("(?i)penalty"))
                    .and_(pl.col("text").str.contains("(?i)1st down")))
                .then(True)
            ...
            penalty_in_text = pl.when((pl.col("text").str.contains("(?i)penalty"))
            ...
            yds_penalty = pl.when(pl.col("penalty_flag") == True)
                .then(pl.col("penalty_text").str.extract(r"(?i)(.{0,3}) yards|(?i)yds|(?i)yd to the", 1)
            ...
            yds_penalty = pl.when((pl.col("penalty_flag") == True)
                ...
                .then(pl.col("text").str.extract(r"(.{0,4})yards\)|Yards\)|yds\)|Yds\)", 1)
                    .str.replace("yards\\)|Yards\\)|yds\\)|Yds\\)", "").str.replace("\\(", ""))
    ...
    def __add_play_category_flags(self, play_df):
        play_df = play_df.with_columns(
            ...
            sack = pl.when(pl.col("type.text").is_in(["Sack"]))
                .then(True)
                .when((pl.col("type.text").is_in(["Fumble Recovery (Opponent)",
                                                "Fumble Recovery (Opponent) Touchdown",
                                                "Fumble Recovery (Own)",
                                                "Fumble Recovery (Own) Touchdown"]))
                        .and_(pl.col("pass") == True)
                        .and_(pl.col("text").str.contains("(?i)sacked")))
                .then(True)
                .when((pl.col("type.text").is_in(["Safety"]))
                        .and_(pl.col("text").str.contains("(?i)sacked")))
                .then(True)
                .otherwise(False),
            # --- Interceptions ------
            ...
            completion = pl.when(pl.col("type.text").is_in(["Pass Reception", "Pass Completion", "Passing Touchdown"]))
                .then(True)
                .when((pl.col("type.text").is_in(["Fumble Recovery (Opponent)",
                                                "Fumble Recovery (Opponent) Touchdown",
                                                "Fumble Recovery (Own)",
            ...
            pass_attempt = pl.when(pl.col("type.text").is_in(["Pass Reception", "Pass Completion", "Passing Touchdown", "Pass Incompletion"]))
                ...
                .then(True)
                .otherwise(False),
            target = pl.when(pl.col("type.text").is_in(["Pass Reception", "Pass Completion", "Passing Touchdown", "Pass Incompletion"]))
                .then(True)
                .when((pl.col("type.text").is_in(["Fumble Recovery (Opponent)",
                                                    "Fumble Recovery (Opponent) Touchdown",
                                                    "Fumble Recovery (Own)",
                ...
                .then(True)
                .otherwise(False),
            pass_breakup = pl.when(pl.col("text").str.contains("(?i)broken up by"))
                .then(True)
                .otherwise(False),
            # --- Pass/Rush TDs ------
            ...
            downs_turnover = pl.when((pl.col("type.text").is_in(normalplay))
            ...
            yds_punt_gained = pl.when(pl.col("punt") == True)
                .then(pl.col("statYardage"))
                .otherwise(None),
            fg_attempt = pl.when((pl.col("type.text").str.contains(r"(?i)Field Goal"))
                                .or_(pl.col("text").str.contains(r"(?i)Field Goal")))
                .then(True)
                .otherwise(False),
            fg_made = pl.col("type.text") == "Field Goal Good",
            yds_fg = pl.col("text").str.extract(r"(?i)(\d+)\s?Yd Field|(?i)(\d+)\s?YD FG|(?i)(\d+)\s?Yard FG|(?i)(\d+)\s?Field|(?i)(\d+)\s?Yard Field", 0).str.extract(r"(\d+)").cast(pl.Int32),
            ...
            pl.when((pl.col("start.yardsToEndzone").is_null())
                ...
                .when((pl.col("start.yardsToEndzone").is_null())
                    .and_(pl.col("type.text").is_in(kickoff_vec) == False)
            ...
            sp = pl.when((pl.col("fg_attempt") == True)
                        .or_(pl.col("punt") == True)
                        .or_(pl.col("kickoff_play") == True))
                .then(True)
                .otherwise(False),
            play = pl.when(pl.col("type.text").is_in([
                    "Timeout", "End Period", "End of Half", "Penalty"]) == False)
                .then(True)
                .otherwise(False),
        ).with_columns(
            scrimmage_play = pl.when((pl.col("sp") == False)
                                .and_(pl.col("type.text").is_in([
                                    "Timeout", "Extra Point Good", "Extra Point Missed",
                                    "Two-Point Pass", "Two-Point Rush", "Penalty"]) == False))
                .then(True)
                .otherwise(False),
            # --- Change of pos_team by lead('pos_team', 1)----
            change_of_pos_team  = pl.when((pl.col("pos_team") == pl.col("lead_pos_team"))
                                    .and_(((pl.col("lead_play_type").is_in(["End Period", "End of Half"])) == False)
                                        .or_(pl.col("lead_play_type").is_null())))
                .then(False)
                .when((pl.col("pos_team") == pl.col("lead_pos_team2"))
                        .and_((pl.col("lead_play_type").is_in(["End Period", "End of Half"]))
                                .or_(pl.col("lead_play_type").is_null())))
                .then(False)
                .otherwise(True),
            ...
            pos_score_diff_end = pl.when(((pl.col("type.text").is_in(end_change_vec))
    ...
    def __add_yardage_cols(self, play_df):
        play_df = play_df.with_columns(
            yds_rushed = pl.when((pl.col("rush") == True)
                                .and_(pl.col("text").str.contains("(?i)run for no gain")))
                .then(0)
                .when((pl.col("rush") == True)
                                .and_(pl.col("text").str.contains("(?i)for no gain")))
                .then(0)
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)run for a loss of")))
                .then(-1 * pl.col("text").str.extract(r"(?i)run for a loss of (\d+)").cast(pl.Int32))
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)rush for a loss of")))
                .then(-1 * pl.col("text").str.extract(r"(?i)rush for a loss of (\d+)").cast(pl.Int32))
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)run for")))
                .then(pl.col("text").str.extract(r"(?i)run for (\d+)").cast(pl.Int32))
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)rush for")))
                .then(pl.col("text").str.extract(r"(?i)rush for (\d+)").cast(pl.Int32))
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)Yd Run")))
                .then(pl.col("text").str.extract(r"(?i)(\d+) Yd Run").cast(pl.Int32))
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)Yd Rush")))
                .then(pl.col("text").str.extract(r"(?i)(\d+) Yd Rush").cast(pl.Int32))
                .when((pl.col("rush") == True)
                        .and_(pl.col("text").str.contains("(?i)Yard Rush")))
                ...
                .otherwise(None),
            yds_receiving = pl.when((pl.col("pass") == True)
                ...
                .when((pl.col("pass") == True)
                                .and_(pl.col("text").str.contains(r"(?i)incomplete|(?i) sacked|(?i)intercepted|(?i)pass defensed")))
                .then(0)
                .when((pl.col("pass") == True)
                                .and_(pl.col("text").str.contains(r"(?i)incompletion")))
                .then(0)
                .when((pl.col("pass") == True)
                                .and_(pl.col("text").str.contains(r"(?i)Yd pass")))
                .then(pl.col("text").str.extract(r"(?i)(\d+) Yd pass").cast(pl.Int32))
                .otherwise(None),
            yds_int_return = pl.when((pl.col("pass") == True)
                                        .and_(pl.col("int_td") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)Yd Interception Return")))
                .then(pl.col("text").str.extract(r"(?i)(.+)Yd Interception Return")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .when((pl.col("pass") == True)
                ...
                .when((pl.col("pass") == True)
                                        .and_(pl.col("int") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)for a TD")))
                .then(pl.col("text").str.extract(r"(?i)return for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .when((pl.col("pass") == True)
                                        .and_(pl.col("int") == True))
                .then(pl.col("text").str.replace("for a 1st", "")
                        .str.extract(r"(?i)for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .otherwise(None),
            yds_kickoff = pl.when(pl.col("kickoff_play") == True)
                            .then(pl.col("text").str.extract(r"(?i)kickoff for (.+)")
                                .str.extract(r"(\d+)").cast(pl.Int32))
                            .otherwise(None),
            yds_kickoff_return = pl.when((pl.col("kickoff_play") == True)
                                        .and_(pl.col("kickoff_tb") == True)
                ...
                .then(40)
                .when((pl.col("kickoff_downed") == True)
                                        .or_(pl.col("kickoff_fair_catch") == True))
                .then(0)
                .when((pl.col("kickoff_play") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)returned by")))
                .then(pl.col("text").str.extract(r"(?i)returned by (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .when((pl.col("kickoff_play") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)return for")))
                .then(pl.col("text").str.extract(r"(?i)return for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .otherwise(None),
            yds_punted = pl.when((pl.col("punt") == True)
                                .and_(pl.col("punt_blocked") == True))
                .then(0)
                .when(pl.col("punt") == True)
                .then(pl.col("text").str.extract(r"(?i)punt for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .otherwise(None),
            yds_punt_return = pl.when((pl.col("punt") == True)
                                        .and_(pl.col("punt_tb") == True))
                .then(20)
                .when((pl.col("punt") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)fair catch|fair caught")))
                .then(0)
                .when((pl.col("punt") == True)
                                        .and_((pl.col("punt_downed") == True)
                                                .or_(pl.col("punt_oob") == True)
                                                .or_(pl.col("punt_fair_catch") == True)))
                .then(0)
                .when((pl.col("punt") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)no return|no gain")))
                .then(0)
                .when((pl.col("punt") == True)
                                        .and_(pl.col("text").str.contains(r"(?i)returned \d+ yards")))
                .then(pl.col("text").str.extract(r"(?i)returned (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .when((pl.col("punt") == True)
                                        .and_(pl.col("punt_blocked") == False))
                .then(pl.col("text").str.extract(r"(?i)returns for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .when((pl.col("punt") == True)
                                        .and_(pl.col("punt_blocked") == True))
                .then(pl.col("text").str.extract(r"(?i)return for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .otherwise(None),
            yds_fumble_return = pl.when((pl.col("fumble_vec") == True)
                                        .and_(pl.col("kickoff_play") == False))
                .then(pl.col("text").str.extract(r"(?i)return for (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .otherwise(None),
            yds_sacked = pl.when(pl.col("sack") == True)
                .then(-1 * pl.col("text").str.extract(r"(?i)sacked (.+)")
                        .str.extract(r"(\d+)").cast(pl.Int32))
                .otherwise(None),
        ).with_columns(
            yds_penalty = pl.when(pl.col("penalty_detail").is_in(["Penalty Declined", "Penalty Offset"]))
                ...
                .when((pl.col("penalty_detail").is_not_null())
                        .and_(pl.col("yds_penalty").is_null())
                        .and_(pl.col("rush") == True))
    ...
    def __add_player_cols(self, play_df):
        play_df = play_df.with_columns(
            ...
            rush_player = pl.when(pl.col("rush") == True)
                .then(pl.col("text").str.extract(r"(?i)(.{0,25} )run |(?i)(.{0,25} )\d{0,2} Yd Run|(?i)(.{0,25} )rush |(?i)(.{0,25} )rushed ")
            ...
            pass_player = pl.when((pl.col("pass") == True)
                ...
                .then(pl.col("text").str.extract(r"(?i)(.{0,30} )pass |(?i)(.{0,30} )sacked by|(?i)(.{0,30} )sacked for|(?i)(.{0,30} )incomplete|(?i)pass from (.{0,30} ) \( ")
                        .str.replace(r"(?i)pass |(?i) sacked by|(?i) sacked for|(?i) incomplete", ""))
                .when((pl.col("pass") == True)
                                .and_(pl.col("sack_vec") == True)
                                .and_(pl.col("type.text") != "Passing Touchdown"))
                .then(pl.col("text").str.extract(r"(?i)(.{0,30} )sacked by|(?i)(.{0,30} )sacked for")
                        .str.replace(r"(?i)pass |(?i) sacked by|(?i) sacked for|(?i) incomplete", ""))
                .when((pl.col("pass") == True)
                                .and_(pl.col("type.text") == "Passing Touchdown"))
                .then(pl.col("text").str.extract(r"(?i)pass from(.+)")
                        .str.replace(r"pass from", "")
                        # .str.replace(r"\((.+)\)", "")
                        .str.replace(r" \,", ""))
                .otherwise(None)
        ).with_columns(
            pass_player = pl.when((pl.col("type.text") == "Passing Touchdown")
                                .and_(pl.col("pass_player").is_null()))
                .then(pl.col("text").str.extract(r"(.+)pass(.+)? complete to")
            ...
            pass_player = pl.when((pl.col("type.text") == "Passing Touchdown")
                                .and_(pl.col("pass_player").is_null()))
                .then(pl.col("text").str.extract(r"(.+)pass,to")
            ...
            pass_player = pl.when((pl.col("pass") == True)
                                .and_(((pl.col("pass_player").str.strip().str.n_chars() == 0)
                                        .or_(pl.col("pass_player").is_null()))))
                .then("TEAM")
                .otherwise(pl.col("pass_player")),

            # --- WR Names -----
            receiver_player = pl.when((pl.col("pass") == True)
                                    .and_(pl.col("text").str.contains(r"(?i)sacked") == False))
            ...
            receiver_player = pl.when((pl.col("type.text") == "Sack")
                                    .or_(pl.col("type.text") == "Interception Return")
                                    .or_(pl.col("type.text") == "Interception Return Touchdown")
                                    .or_((pl.col("type.text").is_in(["Fumble Recovery (Opponent) Touchdown", "Fumble Recovery (Opponent)"]))
                                            .and_(pl.col("text").str.contains(r"(?i)sacked"))))
                .then(None)
                .otherwise(pl.col("receiver_player").str.replace(r"to ", "")
                            .str.replace(r"(?i)\\,.+", "")
                            .str.replace(r"(?i)for (.+)", "")
            ...
            receiver_player = pl.col("receiver_player").str.replace(r"(?i) &", "")
                                .str.replace(r"(?i)A&M", "")
                                .str.replace(r"(?i) ST", "")
                                .str.replace(r"(?i) GA", "")
                                .str.replace(r"(?i) UL", "")
                                ...
                                .str.replace(r"(?i) \((.+)\)", ""),
            # --- Sack Names -----
            sack_players = pl.when((pl.col("sack") == True)
                        .or_((pl.col("fumble_vec") == True)
                                .and_(pl.col("pass") == True)))
                .then(pl.col("text").str.extract(r"(?i)sacked by(.+)")
                        .str.replace(r"for (.+)", "")
                        .str.replace(r"(.+) by ", "")
            ...
            sack_player2 = pl.when(pl.col("sack_players").str.contains(r"and (.+)"))
                ...
                .otherwise(None),
            # --- Interception Names -----
            interception_player = pl.when(((pl.col("type.text") == "Interception Return")
                                        .or_(pl.col("type.text") == "Interception Return Touchdown"))
                                    .and_(pl.col("pass") == True))
                .then(pl.col("text").str.extract(r"(?i)intercepted (.+)"))
                .when(pl.col("text").str.contains(r"Yd Interception Return"))
                .then(pl.col("text").str.extract(r"(?i)(.{0,25} )\\d{0,2} Yd Interception Return|(?i)(.{0,25} )\\d{0,2} yd interception return")
            ...
            pass_breakup_player = pl.when(pl.col("pass") == True)
                .then(pl.col("text").str.extract(r"(?i)broken up by (.+)")
            ...
            punter_player = pl.when(pl.col("type.text").str.contains("Punt"))
                .then(pl.col("text").str.extract(r"(?i)(.{0,30}) punt|(?i)Punt by (.{0,30})")
            ...
            punt_return_player = pl.when(pl.col("type.text").str.contains("Punt"))
                .then(pl.col("text").str.extract(r"(?i), (.{0,25}) returns|(?i)fair catch by (.{0,25})|(?i), returned by (.{0,25})|(?i)yards by (.{0,30})|(?i) return by (.{0,25})")
                        .str.replace(r"(?i), ", "")
            ...
            punt_block_player = pl.when(pl.col("type.text").str.contains("Punt"))
                .then(pl.col("text").str.extract(r"(?i)punt blocked by (.{0,25})|(?i)blocked by(.+)")
            ...
            punt_block_player = pl.when((pl.col("type.text").str.contains(r"(?i)yd return of blocked punt")))
                .then(pl.col("text").str.extract(r"(?i)(.+) yd return of blocked")
            ...
            punt_block_return_player = pl.when((pl.col("type.text").str.contains(r"Punt"))
                                            .and_(pl.col("text").str.contains(r"(?i)blocked"))
                                            .and_(pl.col("text").str.contains(r"(?i)return")))
                .then(pl.col("text").str.extract(r"(?i)(.+) return"))
                .otherwise(None)
        ).with_columns(
            punt_block_return_player = pl.struct(['punt_block_player', 'punt_block_return_player'])
                .apply(lambda cols: cols['punt_block_return_player'].str.replace(r"(?i)(.+)blocked by", "")
                    .str.replace(pl.format(r"(?i)blocked by {}", cols['punt_block_player']), ""),
                    return_dtype = pl.Utf8)
        ).with_columns(
            punt_block_return_player = pl.col("punt_block_return_player").str.replace(r"(?i)return(.+)", "")
                .str.replace(r"(?i)return", "")
            ...
            kickoff_player = pl.when(pl.col("type.text").str.contains(r"(?i)kickoff"))
                .then(pl.col("text").str.extract(r"(?i)(.{0,25}) kickoff|(.{0,25}) on-side")
            ...
            kickoff_return_player = pl.when(pl.col("type.text").str.contains(r"(?i)ickoff"))
                .then(pl.col("text").str.extract(r"(?i), (.{0,25}) return|(?i), (.{0,25}) fumble|(?i)returned by (.{0,25})|(?i)touchback by (.{0,25})")
                        .str.replace(r", ", "")
            ...
            fg_kicker_player = pl.when(pl.col("type.text").str.contains(r"(?i)Field Goal"))
                .then(pl.col("text").str.extract(r"(?i)(.{0,25} )\\d{0,2} yd field goal|(?i)(.{0,25} )\\d{0,2} yd fg|(?i)(.{0,25} )\\d{0,2} yard field goal")
            ...
            fg_block_player = pl.when(pl.col("type.text").str.contains(r"(?i)Field Goal"))
                .then(pl.col("text").str.extract(r"(?i)blocked by (.{0,25})")
                        .str.replace(r",(.+)", "")
                        .str.replace(r"blocked by ", "")
                        .str.replace(r"  (.)+", ""))
                .otherwise(None),
            # --- Field Goal Returner Names ----
            fg_return_player = pl.when((pl.col("type.text").str.contains(r"(?i)Field Goal"))
                                    .and_(pl.col("text").str.contains(r"(?i)blocked by|missed"))
                                    .and_(pl.col("text").str.contains(r"(?i)return")))
                .then(pl.col("text").str.extract(r"(?i)  (.+)")
                        .str.replace(r"(?i),(.+)", "")
                        .str.replace(r"(?i)return ", "")
                        .str.replace(r"(?i)returned ", "")
                        .str.replace(r"(?i) for (.+)", "")
            ...
            fg_return_player = pl.when((pl.col("type.text").is_in(["Missed Field Goal Return", "Missed Field Goal Return Touchdown"])))
                .then(pl.col("text").str.extract(r"(?i)(.+)return")
                        .str.replace(r"(?i) return", "")
                        .str.replace(r"(?i)(.+),", ""))
            ...
            fumble_player = pl.when(pl.col("text").str.contains(r"(?i)fumble"))
                .then(pl.col("text").str.extract(r"(?i)(.{0,25} )fumble|(?i)(.{0,25} )fumble")
            ...
            # --- Forced Fumble Names ----
            fumble_forced_player = pl.when((pl.col("text").str.contains(r"(?i)fumble"))
                                        .and_(pl.col("text").str.contains(r"(?i)forced by")))
                .then(pl.col("text").str.extract(r"(?i)forced by(.{0,25})")
            ...
            fumble_recovered_player = pl.when((pl.col("text").str.contains(r"(?i)fumble"))
                                            .and_(pl.col("text").str.contains(r"(?i)recovered by")))
                .then(pl.col("text").str.extract(r"(?i)recovered by(.{0,30})")
    ...
    def __after_cols(self, play_df):
        play_df = play_df.with_columns(
            new_down = pl.when(pl.col("type.text") == "Timeout")
                ...
                .when((pl.col("type.text").is_in(penalty))
                        .and_(pl.col("penalty_1st_conv") == True))
                .then(1)
                .when((pl.col("type.text").is_in(penalty))
                        .and_(pl.col("penalty_1st_conv") == False)
            ...
            new_distance = pl.when(pl.col("type.text") == "Timeout")
                ...
                .then(pl.col("start.distance"))
                .otherwise(pl.col("start.distance")),
            middle_8 = pl.when((pl.col("start.adj_TimeSecsRem") >= 1560)
                            .and_(pl.col("start.adj_TimeSecsRem") <= 2040))
                .then(True)
                .otherwise(False),
            rz_play = pl.when(pl.col("start.yardLine") <= 20)
                .then(True)
                .otherwise(False),
            under_2 = pl.when(pl.col("start.TimeSecsRem") <= 120)
                .then(True)
                .otherwise(False),
            goal_to_go = pl.when(pl.col("start.yardLine") <= 10)
                .then(True)
                .otherwise(False),
            scoring_opp = pl.when(pl.col("start.yardLine") <= 40)
                .then(True)
                .otherwise(False),
            stuffed_run = pl.when((pl.col("type.text") == "Rush")
                                .and_(pl.col("yds_rushed") <= 0))
                .then(True)
                .otherwise(False),
            stopped_run = pl.when((pl.col("type.text") == "Rush")
                                .and_(pl.col("yds_rushed") <= 2))
                .then(True)
                .otherwise(False),
            opportunity_run = pl.when((pl.col("type.text") == "Rush")
                                    .and_(pl.col("yds_rushed") <= 4))
                .then(True)
                .otherwise(False),
            highlight_run = pl.when((pl.col("type.text") == "Rush")
                                    .and_(pl.col("yds_rushed") >= 8))
                .then(True)
                .otherwise(False),
            adj_rush_yardage = pl.when((pl.col("type.text") == "Rush")
                                    .and_(pl.col("yds_rushed") > 8))
                .then(8)
                .when((pl.col("type.text") == "Rush")
                        .and_(pl.col("yds_rushed") <= 8))
                .then(pl.col("yds_rushed"))
                .otherwise(None),
        ).with_columns(
            line_yards =  pl.when((pl.col("rush") == True)
                                .and_(pl.col("yds_rushed") < 0))
                .then(1.2 * pl.col("adj_rush_yardage"))
                .when((pl.col("rush") == True)
                        .and_(pl.col("yds_rushed") >= 0)
                        .and_(pl.col("yds_rushed") <= 3))
                ...
                .then(5.5)
                .otherwise(None),
            second_level_yards = pl.when((pl.col("rush") == True)
                                        .and_(pl.col("yds_rushed") >= 4))
                .then(0.5 * (pl.col("adj_rush_yardage") - 4))
                .when(pl.col("rush") == True)
                .then(0)
                .otherwise(None),
            open_field_yards = pl.when((pl.col("rush") == True)
                                        .and_(pl.col("yds_rushed") > 8))
            ...
            opp_highlight_yards = pl.when(pl.col("opportunity_run") == True)
                ...
                .when((pl.col("opportunity_run") == False)
                        .and_(pl.col("rush") == True))
                .then(0)
                .otherwise(None),
            short_rush_success = pl.when((pl.col("start.distance") < 2)
                                        .and_(pl.col("rush") == True)
                                        .and_(pl.col("statYardage") >= pl.col("start.distance")))
                .then(True)
                .when((pl.col("start.distance") < 2)
                                        .and_(pl.col("rush") == True)
                                        .and_(pl.col("statYardage") < pl.col("start.distance")))
                .then(False)
                .otherwise(None),
            short_rush_attempt = pl.when((pl.col("start.distance") < 2)
                                        .and_(pl.col("rush") == True))
                .then(True)
                .when((pl.col("start.distance") >= 2)
                                        .and_(pl.col("rush") == True))
                .then(False)
                .otherwise(None),
            power_rush_success = pl.when((pl.col("start.distance") < 2)
                                        .and_(pl.col("rush") == True)
                ...
                .then(False)
                .otherwise(None),
            power_rush_attempt = pl.when((pl.col("start.distance") < 2)
                                        .and_(pl.col("rush") == True)
                                        .and_(pl.col("start.down").is_in([3, 4])))
                .then(True)
                .when((pl.col("start.distance") < 2)
                ...
                .then(False)
                .otherwise(None),
            early_down = pl.when(((pl.col("down_1") == True)
                                .or_(pl.col("down_2") == True))
                                .and_(pl.col("scrimmage_play") == True))
                .then(True)
                .otherwise(False),
            late_down = pl.when(((pl.col("down_3") == True)
                                .or_(pl.col("down_4")))
                                .and_(pl.col("scrimmage_play") == True))
                .then(True)
                .otherwise(False),
        ).with_columns(
            early_down_pass = pl.when((pl.col("pass") == True)
                                    .and_(pl.col("early_down") == True))
                .then(True)
                .otherwise(False),
            early_down_rush = pl.when((pl.col("rush") == True)
                                    .and_(pl.col("early_down") == True))
                .then(True)
                .otherwise(False),
            late_down_pass = pl.when((pl.col("pass") == True)
                                    .and_(pl.col("late_down") == True))
                .then(True)
                .otherwise(False),
            late_down_rush = pl.when((pl.col("rush") == True)
                                    .and_(pl.col("late_down") == True))
                .then(True)
                .otherwise(False),
            standard_down = pl.when((pl.col("scrimmage_play") == True)
                                    .and_(pl.col("down_1") == True))
                .then(True)
                .when((pl.col("scrimmage_play") == True)
                ...
                .then(True)
                .otherwise(False),
            passing_down = pl.when((pl.col("scrimmage_play") == True)
                                    .and_(pl.col("down_2") == True)
                                    .and_(pl.col("start.distance") >= 8))
                .then(True)
                .when((pl.col("scrimmage_play") == True)
                ...
                .then(True)
                .otherwise(False),
            TFL = pl.when((pl.col("type.text") != "Penalty")
                        .and_(pl.col("sp") == False)
            ...
            TFL_pass = pl.when((pl.col("TFL") == True)
                            .and_(pl.col("pass") == True))
                .then(True)
                .otherwise(False),
            TFL_rush = pl.when((pl.col("TFL") == True)
                            .and_(pl.col("rush") == True))
                .then(True)
                .otherwise(False),
    ...
    def __add_spread_time(self, play_df):
        play_df = play_df.with_columns(
            ...
            (pl.col("start.pos_team_spread") * np.exp(-4 * pl.col("start.elapsed_share"))).alias("start.spread_time"),
            pl.when(pl.col("end.pos_team.id") == pl.col("homeTeamId"))
                    .then(pl.col("homeTeamSpread"))
                    .otherwise(-1 * pl.col("homeTeamSpread"))
    ...
    def __process_epa(self, play_df):
        play_df = play_df.with_columns(
            down = pl.when(pl.col("type.text").is_in(kickoff_vec))
                    .then(1)
                    .otherwise(pl.col("start.down")),
            down_1 = pl.when(pl.col("type.text").is_in(kickoff_vec))
                    .then(True)
                    .otherwise(pl.col("down_1")),
            down_2 = pl.when(pl.col("type.text").is_in(kickoff_vec))
                    .then(False)
                    .otherwise(pl.col("down_2")),
            down_3 = pl.when(pl.col("type.text").is_in(kickoff_vec))
                    .then(False)
                    .otherwise(pl.col("down_3")),
            down_4 = pl.when(pl.col("type.text").is_in(kickoff_vec))
                    .then(False)
                    .otherwise(pl.col("down_4")),
            distance = pl.when(pl.col("type.text").is_in(kickoff_vec))
                    .then(10)
                    .otherwise(pl.col("start.distance")),
        ).with_columns(
            pl.when(pl.col("type.text").is_in(kickoff_vec))
        ...
        kick = "kick)"
        play_df = play_df.with_columns(
            EP_start = pl.when(pl.col("type.text").is_in([
                    "Extra Point Good",
                    "Extra Point Missed",
            ...
            .otherwise(pl.col("EP_start")),
                    ...
                    # End of Half
            EP_end = pl.when((pl.col("type.text").str.to_lowercase().str.contains(r"end of game"))
                            .or_(pl.col("type.text").str.to_lowercase().str.contains(r"end of half")))
                    .then(0)
                    ...
                    .when((pl.col("type.text").is_in(defense_score_vec))
                            .and_(pl.col("text").str.to_lowercase().str.contains(r"(?i)safety")))
                    .then(-2)
                    ...
                    .when((pl.col("type.text").is_in(defense_score_vec))
                            .and_(pl.col("text").str.to_lowercase().str.contains(r"kick\)")))
                    .then(-7)
                    # Defense TD
                    .when(pl.col("type.text").is_in(defense_score_vec))
                    .then(-6.92)
                    ...
                    .when((pl.col("type.text").is_in(["Two Point Pass", "Two Point Rush"]))
                            .and_(pl.col("text").str.to_lowercase().str.contains(r"(?i)no good")))
                    .then(0)
                    # Two Point Pass/Rush Good (Pre-2014 Data)
                    .when((pl.col("type.text").is_in(["Two Point Pass", "Two Point Rush"]))
                            .and_(pl.col("text").str.to_lowercase().str.contains(r"(?i)no good") == False))
                    .then(2)
                    # Blocked PAT
                    .when(pl.col("type.text").is_in(["Blocked PAT"]))
                    .then(0)
                    # Flips for Turnovers that aren't kickoffs
                    .when(((pl.col("type.text").is_in(end_change_vec))
                            .or_(pl.col("downs_turnover") == True))
                            .and_(pl.col("type.text").is_in(kickoff_vec) == False))
                    ...
                    .then(pl.col("EP_end") * -1)
                    # Onside kicks
                    .when((pl.col("kickoff_onside") == True)
                            .and_(pl.col("change_of_pos_team") == True))
                    .then(pl.col("EP_end") * -1)
            ...
            EP_start = pl.when((pl.col("type.text").is_in(["Timeout", "End Period"]))
                            .and_(pl.col("lag_change_of_pos_team") == False))
            ...
            EPA = pl.when(pl.col("type.text").is_in(["Timeout"]))
                    .then(0)
                    .when((pl.col("scoring_play") == False)
                            .and_(pl.col("end_of_half") == True))
                    .then(-1 * pl.col("EP_start"))
                    .when((pl.col("type.text").is_in(kickoff_vec))
                            .and_(pl.col("penalty_in_text") == True))
            ...
            # --- EPA Summary flags ----
            EPA_scrimmage = pl.when(pl.col("scrimmage_play") == True)
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_rush = pl.when((pl.col("rush") == True)
                            .and_(pl.col("penalty_in_text") == True))
                    .then(pl.col("EPA"))
                    .when((pl.col("rush") == True)
                    ...
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_pass = pl.when(pl.col("pass") == True)
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_explosive = pl.when((pl.col("pass") == True)
                                    .and_(pl.col("EPA") >= 2.4))
                    .then(True)
                    .when(((pl.col("rush") == True)
                            .and_(pl.col("EPA") >= 1.8)))
            ...
            EPA_non_explosive = pl.when(pl.col("EPA_explosive") == False)
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_explosive_pass = pl.when((pl.col("pass") == True)
                                            .and_(pl.col("EPA") >= 2.4))
                    .then(True)
                    .otherwise(False),
            EPA_explosive_rush = pl.when((pl.col("rush") == True)
                                            .and_(pl.col("EPA") >= 1.8))
                    .then(True)
                    .otherwise(False),
            first_down_created = pl.when((pl.col("scrimmage_play") == True)
                                            .and_(pl.col("end.down") == 1)
                                            .and_(pl.col("start.pos_team.id") == pl.col("end.pos_team.id")))
                    .then(True)
                    .otherwise(False),
            EPA_success = pl.when(pl.col("EPA") > 0)
                    .then(True)
                    .otherwise(False),
            EPA_success_early_down = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("early_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_early_down_pass = pl.when((pl.col("pass") == True)
                                                    .and_(pl.col("EPA") > 0)
                                                    .and_(pl.col("early_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_early_down_rush = pl.when((pl.col("rush") == True)
                                                    .and_(pl.col("EPA") > 0)
                                                    .and_(pl.col("early_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_late_down = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("late_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_late_down_pass = pl.when((pl.col("pass") == True)
                                                    .and_(pl.col("EPA") > 0)
                                                    .and_(pl.col("late_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_late_down_rush = pl.when((pl.col("rush") == True)
                                                    .and_(pl.col("EPA") > 0)
                                                    .and_(pl.col("late_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_standard_down = pl.when((pl.col("EPA") > 0)
                                                    .and_(pl.col("standard_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_passing_down = pl.when((pl.col("EPA") > 0)
                                                    .and_(pl.col("passing_down") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_pass = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("pass") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_rush = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("rush") == True))
                    .then(True)
                    .otherwise(False),
            EPA_success_EPA = pl.when(pl.col("EPA") > 0)
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_success_standard_down_EPA = pl.when((pl.col("EPA") > 0)
                                                    .and_(pl.col("standard_down") == True))
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_success_passing_down_EPA = pl.when((pl.col("EPA") > 0)
                                                    .and_(pl.col("passing_down") == True))
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_success_pass_EPA = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("pass") == True))
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_success_rush_EPA = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("rush") == True))
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_middle_8_success = pl.when((pl.col("EPA") > 0)
                                            .and_(pl.col("middle_8") == True))
                    .then(True)
                    .otherwise(False),
            EPA_middle_8_success_pass = pl.when((pl.col("pass") == True)
                                                    .and_(pl.col("EPA") > 0)
                                                    .and_(pl.col("middle_8") == True))
                    .then(True)
                    .otherwise(False),
            EPA_middle_8_success_rush = pl.when((pl.col("rush") == True)
                                                    .and_(pl.col("EPA") > 0)
                                                    .and_(pl.col("middle_8") == True))
                    .then(True)
                    .otherwise(False),
            EPA_penalty = pl.when(pl.col("type.text").is_in(["Penalty", "Penalty (Kickoff)"]))
                    ...
                    .then(pl.col("EP_end") - pl.col("EP_start"))
                    .otherwise(None),
            EPA_sp = pl.when((pl.col("fg_attempt") == True)
                            .or_(pl.col("punt") == True)
                            .or_(pl.col("kickoff_play") == True))
                    .then(pl.col("EPA"))
                    .otherwise(False),
            EPA_fg = pl.when(pl.col("fg_attempt") == True)
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_punt = pl.when(pl.col("punt") == True)
                    .then(pl.col("EPA"))
                    .otherwise(None),
            EPA_kickoff = pl.when(pl.col("kickoff_play") == True)
                    .then(pl.col("EPA"))
                    .otherwise(None),
        )
        return play_df
    ...
    def __process_qbr(self, play_df):
        play_df = play_df.with_columns(
            ...
            weight = pl.when(pl.col("home_wp_before") < 0.1)
                    .then(0.6)
                    .when((pl.col("home_wp_before") >= 0.1)
                            .and_(pl.col("home_wp_before") < 0.2))
                    .then(0.9)
                    .when((pl.col("home_wp_before") >= 0.8)
                            .and_(pl.col("home_wp_before") < 0.9))
                    .then(0.9)
                    .when(pl.col("home_wp_before") > 0.9)
                    .then(0.6)
                    .otherwise(1),
            non_fumble_sack = pl.when((pl.col("sack_vec") == True)
                                    .and_(pl.col("fumble_vec") == False))
                    .then(True)
                    .otherwise(False),
        ).with_columns(
            sack_epa = pl.when(pl.col("non_fumble_sack") == True)
                    .then(pl.col("qbr_epa"))
                    .otherwise(None),
            pass_epa = pl.when(pl.col("pass") == True)
                    .then(pl.col("qbr_epa"))
                    .otherwise(None),
            rush_epa = pl.when(pl.col("rush") == True)
                    .then(pl.col("qbr_epa"))
                    .otherwise(None),
            pen_epa = pl.when(pl.col("penalty_flag") == True)
                    .then(pl.col("qbr_epa"))
                    .otherwise(None),
        ).with_columns(
            sack_weight = pl.when(pl.col("non_fumble_sack") == True)
                    .then(pl.col("weight"))
                    .otherwise(None),
            pass_weight = pl.when(pl.col("pass") == True)
                    .then(pl.col("weight"))
                    .otherwise(None),
            rush_weight = pl.when(pl.col("rush") == True)
                    .then(pl.col("weight"))
                    .otherwise(None),
            pen_weight = pl.when(pl.col("penalty_flag") == True)
                    .then(pl.col("weight"))
                    .otherwise(None),
        ).with_columns(
    ...
    def __process_wpa(self, play_df):
        ...
        play_df = play_df.with_columns(
            ...
            (pl.col("start.ExpScoreDiff_touchback") / (pl.col("start.adj_TimeSecsRem") + 1))
                    .alias("start.ExpScoreDiff_Time_Ratio_touchback"),
            (pl.col("start.ExpScoreDiff") / (pl.col("start.adj_TimeSecsRem") + 1))
                    .alias("start.ExpScoreDiff_Time_Ratio"),
            # ---- prepare variables for wp_after calculations ----
            pl.when(((pl.col("type.text").is_in(end_change_vec))
                    .or_(pl.col("downs_turnover") == True))
                    .and_(pl.col("kickoff_play") == False)
            ...
            (pl.col("end.ExpScoreDiff") / (pl.col("end.adj_TimeSecsRem") + 1))
        ...
        play_df = play_df.with_columns(
            ...
            wp_after = pl.when(pl.col("type.text").is_in(["Timeout"]))
                    .then(pl.col("wp_before"))
                    .when((pl.col("status_type_completed") == True)
                            .and_((pl.col("lead_play_type").is_null())
                                    .or_(pl.col("game_play_number") == pl.col("game_play_number").max()))
                    ...
                    .when((pl.col("lead_play_type").is_in(["End Period", "End of Half"]))
                            .and_(pl.col("change_of_pos_team") == False))
                    .then(pl.col("lead_wp_before"))
                    .when((pl.col("lead_play_type").is_in(["End Period", "End of Half"]))
                            .and_(pl.col("change_of_pos_team") == True))
    ...
    def __add_drive_data(self, play_df):
        ...
        play_df = play_df.with_columns(
            ...
            drive_stopped = pl.when(pl.col("drive.result").is_null())
                    .then(False)
                    .otherwise(pl.col("drive.result").str.to_lowercase().str.contains(r"(?i)punt|fumble|interception|downs")),
        ).with_columns(
            drive_start = pl.col("drive_start").cast(pl.Float32),
        ).with_columns(
            drive_play_index = pl.col("scrimmage_play").cumsum().over("drive.id"),
        ).with_columns(
            drive_offense_plays = pl.when((pl.col("sp") == False)
                    .and_(pl.col("scrimmage_play") == True))
            ...
            drive_offense_yards = pl.when((pl.col("sp") == False)
                    .and_(pl.col("scrimmage_play") == True))
                    .then(pl.col("statYardage"))
                    .otherwise(0),
        ).with_columns(
    ...
    def create_box_score(self, play_df):
        ...
        passer_box = pass_box.fill_null(0.0).groupby(by=["pos_team","passer_player_name"]).agg(
        ...
        pass_qbr = pass_qbr_box.groupby(by=["pos_team","athlete_name"]).agg(
        ...
        pass_qbr = pass_qbr.with_columns(
                exp_qbr = pl.lit(qbr_result)
        )
        passer_box = passer_box.join(pass_qbr,
                                    left_on=["passer_player_name", "pos_team"],
                                    right_on=["athlete_name", "pos_team"])

        rusher_box = rush_box.fill_null(0.0).groupby(by=["pos_team","rusher_player_name"]).agg(
            Car= pl.col('rush').sum(),
        ...
        receiver_box = pass_box.fill_null(0.0).groupby(by=["pos_team","receiver_player_name"]).agg(
        ...
        team_base_box = play_df.groupby(by=["pos_team"]).agg(
        ...
        team_pen_box = play_df.filter(pl.col("penalty_flag") == True).groupby(by=["pos_team"]).agg(
        ...
        team_scrimmage_box = play_df.filter(pl.col("scrimmage_play") == True).groupby(by=["pos_team"]).agg(
        ...
        team_sp_box = play_df.filter(pl.col("sp") == True).groupby(by=["pos_team"]).agg(
        ...
        team_scrimmage_box_pass = play_df.filter(
            (pl.col("pass") == True) & (pl.col("scrimmage_play") == True)
        ).fill_null(0.0).groupby(by=["pos_team"]).agg(
        ...
        team_scrimmage_box_rush = play_df.filter(
            (pl.col("rush") == True) & (pl.col("scrimmage_play") == True)
        ).fill_null(0.0).groupby(by=["pos_team"]).agg(
        ...
        team_rush_base_box = play_df.filter(
            (pl.col("scrimmage_play") == True)
        ).fill_null(0.0).groupby(by=["pos_team"]).agg(
        ...
        team_rush_power_box = play_df.filter(
            (pl.col("power_rush_attempt") == True) & (pl.col("scrimmage_play") == True)
        ).fill_null(0.0).groupby(by=["pos_team"]).agg(
        ...
        team_rush_opp_box = play_df.filter(
        ...
        team_data_frames = [team_rush_opp_box, team_pen_box, team_sp_box, team_scrimmage_box_rush, team_scrimmage_box_pass,
                            team_scrimmage_box, team_base_box, team_rush_base_box, team_rush_power_box, team_rush_box]
        team_box = reduce(lambda left, right: left.join(right, on=["pos_team"], how="outer"), team_data_frames)

        situation_box_normal = play_df.filter(pl.col("scrimmage_play") == True).groupby(by=["pos_team"]).agg(
        ...
        situation_box_rz = play_df.filter(
            (pl.col("rz_play") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_third = play_df.filter(
            (pl.col("start.down") == 3) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_pass = play_df.filter(
            (pl.col("pass") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_rush = play_df.filter(
            (pl.col("rush") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_middle8 = play_df.filter(
            (pl.col("middle_8") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_middle8_pass = play_df.filter(
        ...
        situation_box_middle8_rush = play_df.filter(
        ...
        situation_box_early = play_df.filter(
            (pl.col("early_down") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_early_pass = play_df.filter(
        ...
        situation_box_early_rush = play_df.filter(
        ...
        situation_box_late = play_df.filter(
            (pl.col("late_down") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_standard = play_df.filter(
            (pl.col("standard_down") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_box_passing = play_df.filter(
            (pl.col("passing_down") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["pos_team"]).agg(
        ...
        situation_data_frames = [
            situation_box_normal, situation_box_pass, situation_box_rush,
            situation_box_rz, situation_box_third, situation_box_early, situation_box_early_pass,
            situation_box_early_rush, situation_box_middle8, situation_box_middle8_pass,
            situation_box_middle8_rush, situation_box_late, situation_box_standard, situation_box_passing
        ]

        situation_box = reduce(lambda left, right: left.join(right, on=["pos_team"], how="outer"), situation_data_frames)
        ...
        def_base_box = play_df.filter(pl.col("scrimmage_play") == True).groupby(by=["def_pos_team"]).agg(
            scrimmage_plays = pl.col('scrimmage_play').sum(),
            TFL = pl.col('TFL').sum(),
        ...
        def_box_havoc_pass = play_df.filter(
            (pl.col("pass") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["def_pos_team"]).agg(
        ...
        def_box_havoc_rush = play_df.filter(
            (pl.col("rush") == True) & (pl.col("scrimmage_play") == True)
        ).groupby(by=["def_pos_team"]).agg(
        ...
        turnover_box = play_df.filter(pl.col("scrimmage_play") == True).groupby(by=["pos_team"]).agg(
            pass_breakups = pl.col('pass_breakup').sum(),
        ...
        drives_data = play_df.filter(pl.col("scrimmage_play") == True).groupby(by=["pos_team"]).agg(
    ...
    def run_processing_pipeline(self):
        if self.ran_pipeline == False:
            ...
            if pbp_json.get('header').get('competitions')[0].get('playByPlaySource') != 'none':
                self.plays_json = self.plays_json.pipe(
