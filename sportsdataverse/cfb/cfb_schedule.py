...
def espn_cfb_schedule(
    dates=None, week=None, season_type=None, groups=None, limit=500
) -> pd.DataFrame:
    ...
    url = "http://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?{}{}{}{}{}{}".format(
        limit_url, groups, dates, week, season_type, cache_buster_url
    )
...
def espn_cfb_calendar(season=None, groups=None, ondays=None) -> pd.DataFrame:
    ...
    if ondays is not None:
        url = "https://sports.core.api.espn.com/v2/sports/football/leagues/college-football/seasons/{}/types/2/calendar/ondays".format(
            season
        )
        resp = download(url=url)
        if resp is not None:
                ...
                for i in range(len(txt)):
                    if txt[i].get("entries", None) is not None:
                        ...
                        full_schedule = pd.concat(
                            [full_schedule, reg], ignore_index=True
                        )
                if not full_schedule.empty:
                    full_schedule["season"] = season
                    full_schedule.columns = [
                        underscore(c) for c in full_schedule.columns.tolist()
                    ]
                    full_schedule = full_schedule.rename(
                        columns={
                            "week_value": "week",
                            "season_type_value": "season_type",
                        }
                    )
