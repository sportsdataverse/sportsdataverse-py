...
def espn_nfl_game_rosters(game_id: int, raw = False, return_as_pandas = True, **kwargs) -> pd.DataFrame:
    ...
    summary_url = "https://sports.core.api.espn.com/v2/sports/football/leagues/nfl/events/{x}/competitions/{x}/competitors".format(x=game_id)
    ...
    teams_df = helper_nfl_team_items(items = items, **kwargs)
    teams_rosters = team_rosters.join(teams_df, how = 'left',on = 'team_id')
    athletes = helper_nfl_athlete_items(teams_rosters = team_rosters, **kwargs)
