...
class NFLPlayProcess(object):
    ...
    def __helper_nfl_pbp(self, pbp_txt):
        gameSpread, overUnder, homeFavorite, gameSpreadAvailable = (
            self.__helper_nfl_pickcenter(pbp_txt)
        )
        pbp_txt["timeouts"] = {}
        ...
        pbp_txt["boxscoreSource"] = pbp_txt["header"]["competitions"][0][
            "boxscoreSource"
        ]
        pbp_txt["gameSpreadAvailable"] = gameSpreadAvailable
        pbp_txt["gameSpread"] = gameSpread
    ...
    def __add_drive_data(self, play_df):
        ...
        play_df["drive_play_index"] = (
            base_groups["scrimmage_play"]
        ...
        play_df["prog_drive_EPA"] = (
            base_groups["EPA_scrimmage"]
            .apply(lambda x: x.cumsum())
            .reset_index(level=0, drop=True)
        )
        play_df["prog_drive_WPA"] = (
            base_groups["wpa"]
            .apply(lambda x: x.cumsum())
            .reset_index(level=0, drop=True)
        )
        ...
        play_df["drive_total_yards"] = (
            play_df.groupby(["drive.id"])["drive_offense_yards"]
            .apply(lambda x: x.cumsum())
            .reset_index(level=0, drop=True)
        )
        return play_df
    ...
    def create_box_score(self):
        ...
        total_fumbles = reduce(
            lambda x, y: x + y,
            map(lambda x: x.get("total_fumbles", 0), turnover_box_json),
        )
