...
# "no_score" : float(p[6])
ep_model_file = pkg_resources.resource_filename("sportsdataverse", "nfl/models/ep_model.model")
wp_spread_file = pkg_resources.resource_filename("sportsdataverse", "nfl/models/wp_spread.model")
qbr_model_file = pkg_resources.resource_filename("sportsdataverse", "nfl/models/qbr_model.model")
...
class NFLPlayProcess(object):
    ...
    def __helper_nfl_pbp_drives(self, pbp_txt):
        ...
        if (
            ...
            pbp_txt = self.__helper_nfl_pbp_features(
                pbp_txt,
                gameSpread,
                gameSpreadAvailable,
                overUnder,
    ...
    def __helper_nfl_pbp_features(
        ...
        pbp_txt["plays"]["end.team.id"] = (
            pbp_txt["plays"]["end.team.id"].fillna(value=pbp_txt["plays"]["start.team.id"]).apply(lambda x: int(x))
        )
        ...
        pbp_txt["timeouts"][homeTeamId]["1"] = (
            pbp_txt["plays"]
            .loc[(pbp_txt["plays"]["homeTimeoutCalled"] == True) & (pbp_txt["plays"]["period.number"] <= 2)]
            .reset_index()["id"]
        )
        ...
        pbp_txt["timeouts"][awayTeamId]["1"] = (
            pbp_txt["plays"]
            .loc[(pbp_txt["plays"]["awayTimeoutCalled"] == True) & (pbp_txt["plays"]["period.number"] <= 2)]
            .reset_index()["id"]
        )
        ...
        pbp_txt["plays"]["end.homeTeamTimeouts"] = 3 - pbp_txt["plays"].apply(
            ...
            | ((pbp_txt["timeouts"][homeTeamId]["2"] <= x["id"]) & (x["period.number"] > 2)),
            axis=1,
        ...
        pbp_txt["plays"]["end.awayTeamTimeouts"] = 3 - pbp_txt["plays"].apply(
            ...
            | ((pbp_txt["timeouts"][awayTeamId]["2"] <= x["id"]) & (x["period.number"] > 2)),
            axis=1,
        ).apply(lambda x: int(x.sum()), axis=1)
        ...
        pbp_txt["plays"]["start.distance"] = np.where(
            ...
            & (pbp_txt["plays"]["start.downDistanceText"].str.lower().str.contains("goal")),
            pbp_txt["plays"]["start.yardsToEndzone"],
    ...
    def __setup_penalty_data(self, play_df):
        ...
        play_df.loc[
            play_df["text"].str.contains("penalty", case=False, flags=0, na=False, regex=True),
            "penalty_flag",
        ] = True
        ...
        play_df.loc[
            (play_df["type.text"] == "Penalty")
            & (play_df["text"].str.contains("no play", case=False, flags=0, na=False, regex=True)),
            "penalty_no_play",
        ] = True
        play_df.loc[
            (play_df["text"].str.contains("penalty", case=False, flags=0, na=False, regex=True))
        ...
        play_df.loc[
            (play_df["type.text"] == "Penalty")
            & (play_df["text"].str.contains("1st down", case=False, flags=0, na=False, regex=True)),
            "penalty_1st_conv",
        ] = True
        play_df.loc[
            (play_df["text"].str.contains("penalty", case=False, flags=0, na=False, regex=True))
    ...
    def __add_downs_data(self, play_df):
        ...
        play_df = play_df[
            (
                play_df["type.text"].str.contains("end of|coin toss|end period|wins toss", case=False, regex=True)
                == False
            )
        ]
    ...
    def __add_team_score_variables(self, play_df):
        ...
        play_df["homeScore"] = np.select(
            [
                (play_df.scoringPlay == False) & (play_df["game_play_number"] != 1) & (play_df["H_score_diff"] >= 9),
        ...
        play_df["awayScore"] = np.select(
            [
                (play_df.scoringPlay == False) & (play_df["game_play_number"] != 1) & (play_df["A_score_diff"] >= 9),
                (play_df.scoringPlay == False)
                & (play_df["game_play_number"] != 1)
    ...
    def __add_new_play_types(self, play_df):
        ...
        # -- Fix punt return TDs ----
        play_df["type.text"] = np.where(
            (play_df.punt_play == True) & (play_df.td_play == True) & (play_df.td_check == True),
        ...
        play_df["type.text"] = np.where(
            (play_df["type.text"] == "Punt") & (play_df.fumble_vec == True) & (play_df.change_of_poss == 0),
        ...
        play_df["type.text"] = np.where((play_df.kickoff_safety == True), "Kickoff (Safety)", play_df["type.text"])
        play_df["type.text"] = np.where((play_df.punt_safety == True), "Punt (Safety)", play_df["type.text"])
        play_df["type.text"] = np.where((play_df.penalty_safety == True), "Penalty (Safety)", play_df["type.text"])
        play_df["type.text"] = np.where(
    ...
    def __add_play_category_flags(self, play_df):
        ...
        play_df["yds_punted"] = (
            play_df["text"].str.extract(r"(?<= punt for)[^,]+(\d+)", flags=re.IGNORECASE).astype(float)
        )
        ...
        play_df["sp"] = np.where(
            (play_df.fg_attempt == True) | (play_df.punt == True) | (play_df.kickoff_play == True),
            True,
            False,
        )
        play_df["play"] = np.where(
            (~play_df["type.text"].isin(["Timeout", "End Period", "End of Half", "Penalty"])),
            True,
            False,
        )
        ...
        play_df["change_of_pos_team"] = np.where(
            ...
            np.where(
                ...
                & (
                    (play_df.lead_play_type.isin(["End Period", "End of Half"])) | play_df.lead_play_type.isna()
                    == True
                ),
                False,
                True,
        ...
        play_df["fumble_lost"] = np.where((play_df.fumble_vec == True) & (play_df.change_of_poss == True), True, False)
    ...
    def __add_yardage_cols(self, play_df):
        ...
        play_df["yds_rushed"] = np.select(
            [
                (play_df.rush == True)
                & (play_df.text.str.contains("run for no gain", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("for no gain", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("run for a loss of", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("rush for a loss of", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("run for", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("rush for", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("Yd Run", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("Yd Rush", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("Yard Rush", case=False, flags=0, na=False, regex=True)),
                (play_df.rush == True)
                & (play_df.text.str.contains("rushed", case=False, flags=0, na=False, regex=True))
            ...
            [
                ...
                0.0,
                -1
                * play_df.text.str.extract(r"((?<=run for a loss of)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                -1
                * play_df.text.str.extract(r"((?<=rush for a loss of)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<=run for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<=rush for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
        ...
        play_df["yds_receiving"] = np.select(
            ...
            [
                0.0,
                -1
                * play_df.text.str.extract(r"((?<=for a loss of)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
        ...
        play_df["yds_int_return"] = np.select(
            ...
            [
                play_df.text.str.extract(r"(.+) Interception Return", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                0.0,
                -1
                * play_df.text.str.extract(r"((?<= for a loss of)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<= return for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<= return for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
        ...
        play_df["yds_kickoff"] = np.where(
            (play_df["kickoff_play"] == True),
            play_df.text.str.extract(r"((?<= kickoff for)[^,]+)", flags=re.IGNORECASE)[0]
            .str.extract(r"(\d+)")[0]
            .astype(float),
            play_df["yds_kickoff"],
        )
        ...
        play_df["yds_kickoff_return"] = np.select(
            [
                ...
                (play_df.kickoff_play == True)
                & (play_df.fumble_vec == False)
                & (play_df.text.str.contains(r"for no gain|fair catch|fair caught", regex=True, case=False)),
                (play_df.kickoff_play == True)
                & (play_df.fumble_vec == False)
                & (play_df.text.str.contains(r"out-of-bounds|out of bounds", regex=True, case=False)),
            ...
            [
                ...
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<= returned for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
            ],
        ...
        play_df["yds_punted"] = np.select(
            ...
            [
                0,
                play_df.text.str.extract(r"((?<= punt for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
            ],
        ...
        play_df["yds_punt_return"] = np.select(
            [
                ...
                (play_df.punt == True)
                & ((play_df.punt_downed == True) | (play_df.punt_oob == True) | (play_df.punt_fair_catch == True)),
                (play_df.punt == True)
                & (play_df["text"].str.contains(r"no return", case=False, flags=0, na=False, regex=True)),
                (play_df.punt == True)
                & (play_df["text"].str.contains(r"returned \d+ yards", case=False, flags=0, na=False, regex=True)),
            ...
            [
                ...
                0,
                0,
                play_df.text.str.extract(r"((?<= returned)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<= returns for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float),
                play_df.text.str.extract(r"((?<= return for)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
        ...
        play_df["yds_sacked"] = np.select(
            ...
            [
                -1
                * play_df.text.str.extract(r"((?<= sacked)[^,]+)", flags=re.IGNORECASE)[0]
                .str.extract(r"(\d+)")[0]
                .astype(float)
            ],
            default=None,
    ...
    def __add_player_cols(self, play_df):
        ...
        play_df["interception_player"] = np.where(
            (play_df["type.text"] == "Interception Return")
            | (play_df["type.text"] == "Interception Return Touchdown") & play_df["pass"]
            == True,
        ...
        play_df["punt_block_player"] = np.where(
            ...
            play_df.text.str.extract("punt blocked by (.{0,25})| blocked by(.+)", flags=re.IGNORECASE).bfill(axis=1)[
                0
            ],
        ...
        play_df["punt_block_return_player"] = np.where(
            (play_df["type.text"].str.contains("Punt", case=False, flags=0, na=False, regex=True))
            & (
                play_df.text.str.contains("blocked", case=False, flags=0, na=False, regex=True)
                & play_df.text.str.contains("return", case=False, flags=0, na=False, regex=True)
            ),
            play_df.text.str.extract("(.+) return"),
            play_df.punt_block_return_player,
        )
        play_df["punt_block_return_player"] = play_df["punt_block_return_player"].str.replace(
            "(.+)blocked by {punt_block_player}", ""
        )
        play_df["punt_block_return_player"] = play_df["punt_block_return_player"].str.replace(
            "blocked by {punt_block_player}", ""
        )
        ...
        play_df["punt_block_return_player"] = play_df["punt_block_return_player"].str.replace(
            "(.+)blocked by", "", regex=True
        )
        play_df["punt_block_return_player"] = play_df["punt_block_return_player"].str.replace(
            "for a TD(.+)|for a SAFETY(.+)", "", regex=True
        )
        play_df["punt_block_return_player"] = play_df["punt_block_return_player"].str.replace(
            "blocked by", "", regex=True
        )
        ...
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            "for a 1ST down", "", case=False, regex=True
        )
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            "for a 1st down", "", case=False, regex=True
        )
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            "(.+)recovered", "", case=False, regex=True
        )
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            "(.+) by", "", case=False, regex=True
        )
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            ", recove(.+)", "", case=False, regex=True
        )
        ...
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            "a 1st down", "", case=False, regex=True
        )
        play_df["fumble_recovered_player"] = play_df["fumble_recovered_player"].str.replace(
            " a 1st down", "", case=False, regex=True
        )
    ...
    def __after_cols(self, play_df):
        ...
        play_df["middle_8"] = np.where(
            (play_df["start.adj_TimeSecsRem"] >= 1560) & (play_df["start.adj_TimeSecsRem"] <= 2040),
            True,
            False,
        )
        ...
        play_df["short_rush_attempt"] = np.where((play_df["start.distance"] < 2) & (play_df.rush == True), True, False)
        ...
        play_df["power_rush_attempt"] = np.where(
            (play_df["start.distance"] < 2) & (play_df["start.down"].isin([3, 4])) & (play_df.rush == True),
            True,
            False,
        )
        play_df["early_down"] = np.where(
            ((play_df.down_1 == True) | (play_df.down_2 == True)) & (play_df.scrimmage_play == True),
            True,
            False,
        )
        ...
        play_df["early_down_pass"] = np.where((play_df["pass"] == 1) & (play_df.early_down == True), True, False)
        play_df["early_down_rush"] = np.where((play_df["rush"] == 1) & (play_df.early_down == True), True, False)
        play_df["late_down_pass"] = np.where((play_df["pass"] == 1) & (play_df.late_down == True), True, False)
        play_df["late_down_rush"] = np.where((play_df["rush"] == 1) & (play_df.late_down == True), True, False)
        ...
        play_df["TFL_pass"] = np.where((play_df["TFL"] == True) & (play_df["pass"] == True), True, False)
        play_df["TFL_rush"] = np.where((play_df["TFL"] == True) & (play_df["rush"] == True), True, False)
    ...
    def __process_epa(self, play_df):
        ...
        play_df.EP_end = np.select(
            [
                ...
                (
                    play_df["type.text"]
                    .str.lower()
                    .str.contains("end of game", case=False, flags=0, na=False, regex=True)
                )
                | (
                    play_df["type.text"]
                    .str.lower()
                ...
                | (
                    play_df["type.text"]
                    .str.lower()
                    .str.contains("end of half", case=False, flags=0, na=False, regex=True)
                )
                | (
                    play_df["type.text"]
                    .str.lower()
                ...
                (
                    ...
                    & (
                        play_df["type.text"]
                        .str.lower()
                        .str.contains("field goal", case=False, flags=0, na=False, regex=True)
                    )
                    & (
                        play_df["type.text"]
                        .str.lower()
        ...
        play_df["EP_end"] = np.where((play_df["type.text"] == "Timeout"), play_df["EP_start"], play_df["EP_end"])
        ...
        # ----- EPA Summary flags ------
        play_df["EPA_scrimmage"] = np.select([(play_df.scrimmage_play == True)], [play_df.EPA], default=None)
        ...
        play_df["EPA_success_early_down"] = np.where((play_df.EPA > 0) & (play_df.early_down == True), True, False)
        play_df["EPA_success_early_down_pass"] = np.where(
            (play_df["pass"] == True) & (play_df.EPA > 0) & (play_df.early_down == True),
            True,
            False,
        )
        play_df["EPA_success_early_down_rush"] = np.where(
            (play_df["rush"] == True) & (play_df.EPA > 0) & (play_df.early_down == True),
            True,
            False,
        )
        play_df["EPA_success_late_down"] = np.where((play_df.EPA > 0) & (play_df.late_down == True), True, False)
        ...
        play_df["EPA_success_passing_down"] = np.where((play_df.EPA > 0) & (play_df.passing_down == True), True, False)
        play_df["EPA_success_pass"] = np.where((play_df.EPA > 0) & (play_df["pass"] == True), True, False)
        play_df["EPA_success_rush"] = np.where((play_df.EPA > 0) & (play_df.rush == True), True, False)
        ...
        play_df["EPA_success_pass_EPA"] = np.where((play_df.EPA > 0) & (play_df["pass"] == True), play_df.EPA, None)
        play_df["EPA_success_rush_EPA"] = np.where((play_df.EPA > 0) & (play_df.rush == True), True, False)
        play_df["EPA_middle_8_success"] = np.where((play_df.EPA > 0) & (play_df["middle_8"] == True), True, False)
        play_df["EPA_middle_8_success_pass"] = np.where(
            (play_df["pass"] == True) & (play_df.EPA > 0) & (play_df["middle_8"] == True),
            True,
            False,
        )
        play_df["EPA_middle_8_success_rush"] = np.where(
            (play_df["rush"] == True) & (play_df.EPA > 0) & (play_df["middle_8"] == True),
            True,
            False,
        )
        ...
        play_df["EPA_kickoff"] = np.where((play_df.kickoff_play == True), play_df["EPA"], None)
        return play_df
    ...
    def __process_wpa(self, play_df):
        ...
        play_df["start.ExpScoreDiff_Time_Ratio_touchback"] = play_df["start.ExpScoreDiff_touchback"] / (
        ...
        play_df["end.ExpScoreDiff"] = np.select(
            [
                ...
                & (play_df["type.text"].isin(offense_score_vec))
                & (play_df.season <= 2013),
                (play_df["type.text"] == "Timeout") & (play_df["lag_scoringPlay"] == True) & (play_df.season <= 2013),
            ],
            [
        ...
        play_df["wp_after"] = np.select(
            [
                (play_df["type.text"] == "Timeout"),
                game_complete
                & ((play_df.lead_play_type.isna()) | (play_df.game_play_number == max(play_df.game_play_number)))
                ...
                & (play_df["start.def_pos_team.id"] == play_df["end.pos_team.id"]),  # onside recovery
                (play_df["start.pos_team.id"] != play_df["end.pos_team.id"]),
            ],
            [
                play_df.wp_before,
    ...
    def create_box_score(self):
        ...
        passer_box = pd.merge(
            passer_box, pass_qbr, left_on=["passer_player_name", "pos_team"], right_on=["athlete_name", "pos_team"]
        )
        ...
        team_scrimmage_box_pass = (
            self.plays_json[(self.plays_json["pass"] == True) & (self.plays_json["scrimmage_play"] == True)]
            .fillna(0)
            .groupby(by=["pos_team"], as_index=False)
            .agg(
                passes=("pass", sum),
        ...
        team_scrimmage_box_rush = (
            self.plays_json[(self.plays_json["rush"] == True) & (self.plays_json["scrimmage_play"] == True)]
            .groupby(by=["pos_team"], as_index=False)
            .agg(
        ...
        situation_box_middle8 = (
            self.plays_json[(self.plays_json["middle_8"] == True) & (self.plays_json.scrimmage_play == True)]
            .groupby(by=["pos_team"], as_index=False)
            .agg(
        ...
        situation_box_early_rush = (
            self.plays_json[(self.plays_json["rush"] == True) & (self.plays_json.early_down == True)]
            .groupby(by=["pos_team"], as_index=False)
            .agg(
        ...
        situation_box = reduce(
            lambda left, right: pd.merge(left, right, on=["pos_team"], how="outer"), situation_data_frames
        )
        ...
        def_box_havoc_pass = (
            self.plays_json[(self.plays_json.scrimmage_play == True) & (self.plays_json["pass"] == True)]
            .groupby(by=["def_pos_team"], as_index=False)
            .agg(
        ...
        def_box_havoc_rush = (
            self.plays_json[(self.plays_json.scrimmage_play == True) & (self.plays_json["rush"] == True)]
            .groupby(by=["def_pos_team"], as_index=False)
            .agg(
        ...
        drives_data["drive_total_gained_yards_rate"] = (
            100 * drives_data.drive_total_gained_yards / drives_data.drive_total_available_yards
        ).round(2)
