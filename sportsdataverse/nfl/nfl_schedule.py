...
def espn_nfl_schedule(
    dates=None, week=None, season_type=None, limit=500
) -> pd.DataFrame:
    ...
    url = "http://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?{}{}{}{}{}{}".format(
        limit_url, dates, week, season_type, cache_buster_url
    )
...
def espn_nfl_calendar(season=None, ondays=None) -> pd.DataFrame:
    ...
    if ondays is not None:
        url = "https://sports.core.api.espn.com/v2/sports/football/leagues/nfl/seasons/{}/types/2/calendar/ondays".format(
            season
        )
        resp = download(url=url)
        txt = json.loads(resp).get("eventDate").get("dates")
        full_schedule = pd.DataFrame(txt, columns=["dates"])
        full_schedule["datenum"] = list(
        ...
        full_schedule = full_schedule.rename(
            columns={"week_value": "week", "season_type_value": "season_type"}
        )
...
def get_current_week():
    ...
    week1_sep = pd.to_datetime(
        [f"{most_recent_nfl_season()}-09-0{num}" for num in range(1, 8)]
    ).to_series()
