    ...
    if len(search) > 0:
        ...
        if result_count > 0:
            ...
            for i in resp_json["search_player_all"]["queryResults"]["row"]:
                p_df = json_normalize(
                    resp_json["search_player_all"]["queryResults"]["row"]
                )
        ...
        print(
            "To search for MLB players in the MLBAM API, you must include text relating to the player you're searching for."
        )
...
def mlbam_player_teams(playerID: int, season: int):
    ...
    if season > 1 and season < 1860:
        print(
            "Enter a valid season. Baseball wasn't really a thing in the year you specified."
        )
    elif season > 1860:
