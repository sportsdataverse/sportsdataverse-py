...
def retrosplits_player_batting_by_position(
    first_season: int, last_season=None
) -> pd.DataFrame():
    """
    ...
    if first_season < 1974:
        raise SeasonNotFoundError(
            "Batting by position splits are not advalible for seasons before 1974."
        )
        # first_season = 1974
        ...
        print(
            f"The people behind retrosplits do not have a time machine to get stats for {first_season} at this time."
        )
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            ...
            game_log_df = pd.DataFrame()
            print(
                f"Could not get batting by position split stats for the {i} season.\nError:\n{e}"
            )
...
def retrosplits_player_batting_by_runners(
    first_season: int, last_season=None
) -> pd.DataFrame():
    """
    ...
    if first_season < 1974:
        raise SeasonNotFoundError(
            "Batting by runners splits are not advalible for seasons before 1974."
        )
        first_season = 1974
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            ...
            game_log_df = pd.DataFrame()
            print(
                f"Could not get batting by runners split stats for the {i} season.\nError:\n{e}"
            )
        main_df = pd.concat([game_log_df, main_df], ignore_index=True)
    return main_df


def retrosplits_player_batting_by_platoon(
    first_season: int, last_season=None
) -> pd.DataFrame():
    """
    ...
    if first_season < 1974:
        raise SeasonNotFoundError(
            "Batting by platoon splits are not advalible for seasons before 1974."
        )
        # first_season = 1974
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            ...
            game_log_df = pd.DataFrame()
            print(
                f"Could not get batting by platoon split stats for the {i} season.\nError:\n{e}"
            )
        main_df = pd.concat([game_log_df, main_df], ignore_index=True)
    # print(game_log_df)
    return main_df
...
def retrosplits_player_head_to_head_stats(
    first_season: int, last_season=None
) -> pd.DataFrame():
    """
    ...
    if first_season < 1974:
        raise SeasonNotFoundError(
            "Batter vs. pitcher splits are not advalible for seasons before 1974."
        )
        # first_season = 1974
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            ...
            game_log_df = pd.DataFrame()
            print(
                f"Could not get batter vs pitcher split stats for the {i} season.\nError:\n{e}"
            )
        main_df = pd.concat([game_log_df, main_df], ignore_index=True)
    # print(game_log_df)
    return main_df
...
def retrosplits_player_pitching_by_runners(
    first_season: int, last_season=None
) -> pd.DataFrame():
    """
    ...
    if first_season < 1974:
        raise SeasonNotFoundError(
            "Pitching by runners splits are not advalible for seasons before 1974."
        )
        # first_season = 1974
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            ...
            game_log_df = pd.DataFrame()
            print(
                f"Could not get pitching by runners split stats for the {i} season.\nError:\n{e}"
            )
        main_df = pd.concat([game_log_df, main_df], ignore_index=True)
    # print(game_log_df)
    return main_df
...
def retrosplits_player_pitching_by_platoon(
    first_season: int, last_season=None
) -> pd.DataFrame():
    """
    ...
    if first_season < 1974:
        raise SeasonNotFoundError(
            "Pitching by platoon splits are not advalible for seasons before 1974."
        )
        # first_season = 1974
    ...
    for i in tqdm(range(first_season, last_season + 1)):
        ...
        try:
            ...
            game_log_df = pd.DataFrame()
            print(
                f"There was an error getting pitching by platoon split stats for the {i} season.\nError:\n{e}"
            )
