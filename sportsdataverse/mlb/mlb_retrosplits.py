...
def retrosplits_game_logs_player(first_season:int,last_season=0) -> pd.DataFrame():
    """
    Retrives team-level player stats from the Retrosplits project.
    ...
    Returns:
        A pandas dataframe containing team-level stats from historical MLB games.
    """
    now = datetime.now()
...
def retrosplits_game_logs_team(first_season:int,last_season=0) -> pd.DataFrame():
    ...
    Returns:
        A pandas dataframe containing player-level stats from historical MLB games.
    """
    now = datetime.now()
    current_year = int(now.year)
    game_log_df = pd.DataFrame()
...
def retrosplits_player_batting_by_position(first_season:int,last_season=0) -> pd.DataFrame():
    """
    Retrives player-level, batting by position split stats from the Retrosplits project.
    The stats are batting stats, based off of the position the player is pegged to play, at the time of the at-bat.
    The stats returned by this function are season-level stats, not game-level stats.
    ...
    Returns:
        A pandas dataframe containing player-level, batting by position split stats.
    """
