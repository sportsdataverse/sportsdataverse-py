...
def mlbam_teams(season: int, retriveAllStarRosters=False):
    ...
    if season < 1860 or season == None:
        print(
            "1_Please input a proper year. The search will continue with the current year instead."
        )
        season = int(now.year)
        searchURL = searchURL + f"sort_order='name_asc'&season='{season}'"
    elif int(now.year) < season:
        print(
            "0_Please input a proper year. The search will continue with the current year instead."
        )
        season = int(now.year)
        searchURL = searchURL + f"sort_order='name_asc'&season='{season}'"
    else:
...
def mlbam_team_roster(teamID: int, startSeason: int, endSeason: int):
    ...
    try:
        result_count = int(
            resp_json["roster_team_alltime"]["queryResults"]["totalSize"]
        )
    except:
        result_count = 0
    ...
    if result_count > 0:
        ...
        main_df = json_normalize(
            resp_json["roster_team_alltime"]["queryResults"]["row"]
        )
        print("Done")
    else:
        print(
