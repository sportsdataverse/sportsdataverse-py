...
def mlbam_player_season_hitting_stats(playerID: int, season: int, gameType="R"):
    ...
    if len(gameType) > 1:
        print(
            "Check your input for seasonType. Searching for regular season stats instead."
        )
        gameType = "R"
    ...
    if season < 1860 or season == None:
        print(
            "Please input a proper year. The search will continue with the current year instead."
        )
        season = int(now.year)
    ...
    if playerID < 1 or playerID == None or season == None or season < 1860:
        ...
        try:
            result_count = int(
                resp_json["sport_hitting_tm"]["queryResults"]["totalSize"]
            )
        except:
            result_count = 0
        ...
        if result_count > 0:
            ...
            main_df = json_normalize(
                resp_json["sport_hitting_tm"]["queryResults"]["row"]
            )
            print("Done")
        else:
            print(
...
def mlbam_player_season_pitching_stats(playerID: int, season: int, gameType="R"):
    ...
    if playerID < 1 or playerID == None:
        print(
            "You must provide a playerID. Without a proper playerID, this function will not work."
        )
        return None
    else:
        searchURL = searchURL + f"player_id='{playerID}'"
        ...
        try:
            result_count = int(
                resp_json["sport_pitching_tm"]["queryResults"]["totalSize"]
            )
        except:
            result_count = 0
        ...
        if result_count > 0:
            print(f"{result_count} statlines found,\nParsing results into a dataframe.")
            main_df = json_normalize(
                resp_json["sport_pitching_tm"]["queryResults"]["row"]
            )
            print("Done")
        else:
            print(
...
def mlbam_player_career_hitting_stats(playerID: int, gameType="R"):
    ...
    if playerID < 1 or playerID == None:
        ...
        try:
            result_count = int(
                resp_json["sport_career_hitting"]["queryResults"]["totalSize"]
            )
        except:
            result_count = 0
        ...
        if result_count > 0:
            print(f"{result_count} statlines found,\nParsing results into a dataframe.")
            main_df = json_normalize(
                resp_json["sport_career_hitting"]["queryResults"]["row"]
            )
            print("Done")
        else:
            print(
...
def mlbam_player_career_pitching_stats(playerID: int, gameType="R"):
    ...
    if playerID < 1 or playerID == None:
        ...
        try:
            result_count = int(
                resp_json["sport_career_pitching"]["queryResults"]["totalSize"]
            )
        except:
            result_count = 0
        ...
        if result_count > 0:
            print(f"{result_count} statlines found,\nParsing results into a dataframe.")
            main_df = json_normalize(
                resp_json["sport_career_pitching"]["queryResults"]["row"]
            )
            print("Done")
        else:
            print(
