...
def espn_nba_schedule(dates=None, season_type=None, limit=500,
    ...
    for event in events:
        ...
        del_keys = ['geoBroadcasts', 'headlines', 'series', 'situation', 'tickets',
                    'odds', 'broadcasts', 'notes', 'competitors']
        for k in del_keys:
...
def espn_nba_calendar(season=None, ondays=None, return_as_pandas = True, **kwargs) -> pd.DataFrame:
    ...
    if ondays is not None:
        ...
        full_schedule = full_schedule.with_columns(
            url="http://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=" +
                pl.col('dateURL')
        )
