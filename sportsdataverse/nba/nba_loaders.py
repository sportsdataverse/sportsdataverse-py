...
def load_nba_pbp(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            NBA_BASE_URL.format(season=i), engine="auto", columns=None
        )
...
def load_nba_team_boxscore(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2002:
            raise SeasonNotFoundError("season cannot be less than 2002")
        i_data = pd.read_parquet(
            NBA_TEAM_BOX_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_nba_player_boxscore(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2002:
            raise SeasonNotFoundError("season cannot be less than 2002")
        i_data = pd.read_parquet(
            NBA_PLAYER_BOX_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_nba_schedule(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2002:
            raise SeasonNotFoundError("season cannot be less than 2002")
        i_data = pd.read_parquet(
            NBA_TEAM_SCHEDULE_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
