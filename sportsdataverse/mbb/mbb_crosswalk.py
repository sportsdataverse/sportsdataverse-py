...
# ---------------------------------------------------------------------------
# Bundled KenPom team/conference directory.
#
# Provenance: hoopR's ``data-raw/kp_team_info_2026.csv``, generated by that
# repo's ``data-raw/pull_team_links.R`` and shipped in hoopR itself as the
# exported ``hoopR::teams_links`` data object (``pull_team_links.R:93-94``) --
# the exact input the R producer's ``mbb_team_crosswalk()`` reads. It is the
# KenPom *directory* (school name + conference abbreviation per season), not
# ratings: no paid KenPom subscription, credential or request is involved
# here or upstream.
#
# sdv-py vendors it at ``sportsdataverse/mbb/data/kp_team_info.csv`` rather
# than reading the sibling hoopR checkout at runtime (a sibling-path read
# breaks CI on any machine without that checkout). Kept as CSV, not parquet:
# ``mbb/data/*`` is already declared in ``[tool.setuptools.package-data]`` so
# no packaging change is needed, the sibling bundled tables there are CSV, and
# a text file diffs readably when a season is appended. The cost of CSV over
# zstd parquet is ~166 KB of wheel and a single ~10 ms parse on first use
# (lazy + cached below), which is not worth an opaque binary blob.
#
# Trimmed to the three columns the crosswalk joins on (``Team`` / ``Conf`` /
# ``Year``); the four ``*.link`` columns upstream are KenPom URL fragments
# nothing here reads, and dropping them creates no duplicate rows.
#
# Refresh for a future season: re-run hoopR's ``data-raw/pull_team_links.R``
# (needs KenPom login -- a hoopR-maintainer step, not an sdv-py one), then
# from the refreshed ``data-raw/kp_team_info_<year>.csv``::
#
#     import polars as pl
#     pl.read_csv("data-raw/kp_team_info_<year>.csv").select(
#         "Team", "Conf", "Year"
#     ).write_csv("sportsdataverse/mbb/data/kp_team_info.csv")
#
# The file is append-only by season, so the diff is the new year's rows.
# ---------------------------------------------------------------------------
_KP_TEAMS_CACHE: Dict[str, pl.DataFrame] = {}
...
def mbb_schedule_crosswalk(
    ...
    **kwargs: Any,
    ...
    season = int(season) if season is not None else most_recent_mbb_season()
    team_xwalk = mbb_team_crosswalk(season=season, fox=pl.DataFrame(), strict=strict, **kwargs)
    bart_games = bart_super_sked("mbb", season, **kwargs)
    dates = sorted({d for d in bart_games["game_date"].to_list() if d is not None})
    espn_games = espn_scoreboard_games("mbb", dates, strict=strict, **kwargs)
...
def mbb_player_crosswalk(
    ...
    **kwargs: Any,
    ...
    season = int(season) if season is not None else most_recent_mbb_season()
    team_xwalk = mbb_team_crosswalk(season=season, strict=strict, **kwargs)
    frames: List[pl.DataFrame] = []
    espn_tally = FetchTally("espn_mbb_team_roster", strict=strict)
    fox_tally = FetchTally("fox_mbb_team_roster", strict=strict)
    for row in team_xwalk.iter_rows(named=True):
        espn = espn_rosters("mbb", row["espn_team_id"], row["espn_abbreviation"], season, tally=espn_tally, **kwargs)
        if espn.height == 0:
            continue
        fox = fox_rosters("mbb", row["espn_team_id"], row["fox_team_id"], tally=fox_tally, **kwargs)
