...
def espn_mbb_game_rosters(game_id: int, raw=False) -> pd.DataFrame:
    ...
    team_rosters = team_rosters.merge(
        items[["team_id", "order", "home_away", "winner"]], how="left", on="team_id"
    )
    ...
    rosters = athletes.merge(
        teams_rosters, how="left", left_on="athlete_id", right_on="player_id"
    )
...
def helper_mbb_roster_items(items, summary_url):
    ...
    for tm in team_ids:
        ...
        team_roster = pd.json_normalize(
            json.loads(team_roster_resp).get("entries", []), sep="_"
        )
        team_roster.columns = [
            col.replace("$ref", "href") for col in team_roster.columns
        ]
