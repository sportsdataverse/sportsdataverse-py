...
def load_nhl_pbp(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            NHL_BASE_URL.format(season=i), engine="auto", columns=None
        )
...
def load_nhl_schedule(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            NHL_TEAM_SCHEDULE_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_nhl_team_boxscore(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2011:
            raise SeasonNotFoundError("season cannot be less than 2011")
        i_data = pd.read_parquet(
            NHL_TEAM_BOX_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_nhl_player_boxscore(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2011:
            raise SeasonNotFoundError("season cannot be less than 2011")
        i_data = pd.read_parquet(
            NHL_PLAYER_BOX_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
