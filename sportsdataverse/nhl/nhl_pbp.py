...
def espn_nhl_pbp(game_id: int, raw=False) -> Dict:
    ...
    summary_url = "http://site.api.espn.com/apis/site/v2/sports/hockey/nhl/summary?event={}".format(
        game_id
    )
...
def helper_nhl_pbp(game_id, pbp_txt):
    ...
    homeTeamAbbrev = str(
        pbp_txt["header"]["competitions"][0]["competitors"][0]["team"]["abbreviation"]
    )
    awayTeamAbbrev = str(
        pbp_txt["header"]["competitions"][0]["competitors"][1]["team"]["abbreviation"]
    )
...
def helper_nhl_pbp_features(
    ...
    pbp_txt["plays"]["start.period_seconds_remaining"] = 60 * pbp_txt["plays"][
        "clock.minutes"
    ...
    pbp_txt["plays"]["end.period_seconds_remaining"] = pbp_txt["plays"][
        "start.period_seconds_remaining"
    ].shift(1)
    ...
    pbp_txt["plays"]["end.period_seconds_remaining"] = np.select(
        [
            ...
            | (
                (pbp_txt["plays"]["period.number"] == 2)
                & (pbp_txt["plays"]["lag_period.number"] == 1)
            )
            | (
                (pbp_txt["plays"]["period.number"] == 3)
                & (pbp_txt["plays"]["lag_period.number"] == 2)
            )
        ],
        [1200],
