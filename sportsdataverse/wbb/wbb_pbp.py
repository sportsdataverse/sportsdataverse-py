...
def espn_wbb_pbp(game_id: int, raw = False) -> Dict:
    ...
    summary_url = "http://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/summary?event={}".format(game_id)
    summary_resp = download(summary_url)
    summary = json.loads(summary_resp)
    for k in ['plays', 'videos', 'broadcasts', 'pickcenter', 'againstTheSpread', 'odds', 'winprobability', 'teamInfo', 'espnWP', 'leaders']:
    ...
    for k in ['news','shop']:
        if k in pbp_txt.keys():
            del pbp_txt[k]
    incoming_keys_expected = ['boxscore', 'format', 'gameInfo', 'leaders', 'broadcasts',
...
def helper_wbb_pbp_features(game_id, pbp_txt, homeTeamId, awayTeamId, homeTeamMascot, awayTeamMascot, homeTeamName, awayTeamName, homeTeamAbbrev, awayTeamAbbrev, homeTeamNameAlt, awayTeamNameAlt, gameSpread, homeFavorite, gameSpreadAvailable):
    pbp_txt['plays_mod'] = []
