...
def espn_wbb_pbp(game_id: int, raw=False) -> Dict:
    ...
    summary_url = "http://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/summary?event={}".format(
        game_id
    )
    ...
    pbp_json = helper_wbb_pbp(game_id, pbp_txt)
    return pbp_json
...
def helper_wbb_pbp(game_id, pbp_txt):
    gameSpread, overUnder, homeFavorite, gameSpreadAvailable = helper_wbb_pickcenter(
        pbp_txt
    )
    pbp_txt["timeouts"] = {}
    ...
    if pbp_txt["header"]["competitions"][0]["competitors"][0]["homeAway"] == "home":
        ...
        homeTeamName = str(
            pbp_txt["header"]["competitions"][0]["competitors"][0]["team"]["location"]
        )
        homeTeamAbbrev = str(
        ...
        awayTeamName = str(
            pbp_txt["header"]["competitions"][0]["competitors"][1]["team"]["location"]
        )
        awayTeamAbbrev = str(
...
def helper_wbb_pbp_features(
    ...
    pbp_txt["plays"]["start.quarter_seconds_remaining"] = 60 * pbp_txt["plays"][
        "clock.minutes"
    ].astype(int) + pbp_txt["plays"]["clock.seconds"].astype(int)
    ...
    pbp_txt["plays"]["end.quarter_seconds_remaining"] = pbp_txt["plays"][
        "start.quarter_seconds_remaining"
    ].shift(1)
    pbp_txt["plays"]["end.half_seconds_remaining"] = pbp_txt["plays"][
        "start.half_seconds_remaining"
    ].shift(1)
    pbp_txt["plays"]["end.game_seconds_remaining"] = pbp_txt["plays"][
    ...
    pbp_txt["plays"]["end.half_seconds_remaining"] = np.select(
        [
            (pbp_txt["plays"]["game_play_number"] == 1)
            | (
                (pbp_txt["plays"]["game_half"] == "2")
                & (pbp_txt["plays"]["lag_game_half"] == "1")
            )
        ],
        [1200],
...
def helper_wbb_pickcenter(pbp_txt):
    # Spread definition
    if len(pbp_txt.get("pickcenter", [])) > 1:
        homeFavorite = (
            pbp_txt.get("pickcenter", {})[0].get("homeTeamOdds", {}).get("favorite", "")
        )
