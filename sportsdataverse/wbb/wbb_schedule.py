...
def espn_wbb_schedule(
    dates=None, groups=50, season_type=None, limit=500
) -> pd.DataFrame:
    ...
    url = "http://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/scoreboard?limit={}{}{}{}".format(
    ...
    if resp is not None:
        ...
        for event in events:
            ...
            if (
                ...
                event["competitions"][0]["home"]["score"] = (
                    event.get("competitions")[0].get("competitors")[0].get("score")
                )
                ...
                event["competitions"][0]["away"]["score"] = (
                    event.get("competitions")[0].get("competitors")[1].get("score")
                )
            ...
            if len(event.get("competitions")[0]["notes"]) > 0:
                event.get("competitions")[0]["notes_type"] = event.get("competitions")[
                    0
                ]["notes"][0].get("type")
                event.get("competitions")[0]["notes_headline"] = (
                    event.get("competitions")[0]["notes"][0]
                    .get("headline")
                    .replace('"', "")
                )
            else:
...
def espn_wbb_calendar(season=None, ondays=None) -> pd.DataFrame:
    ...
    if ondays is not None:
        url = "https://sports.core.api.espn.com/v2/sports/basketball/leagues/womens-college-basketball/seasons/{}/types/2/calendar/ondays".format(
            season
        )
        resp = download(url=url)
        txt = json.loads(resp).get("eventDate").get("dates")
        full_schedule = pd.DataFrame(txt, columns=["dates"])
        full_schedule["dateURL"] = list(
            map(lambda x: x[:10].replace("-", ""), full_schedule["dates"])
        )
        full_schedule["url"] = (
            "http://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/scoreboard?dates="
        )
        full_schedule["url"] = full_schedule["url"] + full_schedule["dateURL"]
    else:
        url = "http://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/scoreboard?dates={}".format(
            season
        )
        resp = download(url=url)
