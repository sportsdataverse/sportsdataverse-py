...
def load_wnba_pbp(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            WNBA_BASE_URL.format(season=i), engine="auto", columns=None
        )
...
def load_wnba_team_boxscore(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        ...
        i_data = pd.read_parquet(
            WNBA_TEAM_BOX_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_wnba_player_boxscore(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2002:
            raise ValueError("season cannot be less than 2002")
        i_data = pd.read_parquet(
            WNBA_PLAYER_BOX_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
...
def load_wnba_schedule(seasons: List[int]) -> pd.DataFrame:
    ...
    for i in tqdm(seasons):
        if int(i) < 2002:
            raise ValueError("season cannot be less than 2002")
        i_data = pd.read_parquet(
            WNBA_TEAM_SCHEDULE_URL.format(season=i), engine="auto", columns=None
        )
        data = pd.concat([data, i_data], axis=0, ignore_index=True)
    # Give each row a unique index
    data.reset_index(drop=True, inplace=True)
