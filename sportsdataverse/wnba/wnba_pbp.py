...
def espn_wnba_pbp(game_id: int, raw=False) -> Dict:
    ...
    summary_url = "http://site.api.espn.com/apis/site/v2/sports/basketball/wnba/summary?event={}".format(
        game_id
    )
...
def helper_wnba_pbp(game_id, pbp_txt):
    gameSpread, overUnder, homeFavorite, gameSpreadAvailable = helper_wnba_pickcenter(
        pbp_txt
    )
    pbp_txt["timeouts"] = {}
...
def helper_wnba_pbp_features(
    ...
    pbp_txt["plays"]["clock.minutes"] = pbp_txt["plays"]["clock.minutes"].apply(
        lambda x: int(x)
    )

    pbp_txt["plays"]["clock.seconds"] = pbp_txt["plays"]["clock.seconds"].apply(
        lambda x: float(x)
    )
