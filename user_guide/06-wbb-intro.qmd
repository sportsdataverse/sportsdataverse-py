---
title: "WBB tutorial"
description: "WBB walkthrough — the canonical sportsdataverse surface, ported from the Docusaurus tutorials. Code cells show the calls; run them in your own session."
jupyter: python3
---
...
|---|---|---|
| [`espn_wbb_teams`](../reference/index.qmd) | Every D-I program, one wide row each | ESPN |
| [`espn_wbb_team_roster`](../reference/index.qmd) | A team's roster, one row per player | ESPN |
| [`espn_wbb_schedule`](../reference/index.qmd) | Games for a date / date-range | ESPN |
| [`espn_wbb_team_schedule`](../reference/index.qmd) | One program's full season slate | ESPN |
| [`espn_wbb_scoreboard`](../reference/index.qmd) | ⭐ Live + final scoreboard, one row per game | ESPN |
| [`espn_wbb_pbp`](../reference/index.qmd) | Full play-by-play + boxscore for a game | ESPN |
| [`espn_wbb_player_gamelog`](../reference/index.qmd) | A player's game-by-game log | ESPN |
| [`espn_wbb_player_splits`](../reference/index.qmd) | A player's situational stat splits | ESPN |
| [`espn_wbb_team_stats`](../reference/index.qmd) | A team's season stat splits | ESPN |
| [`espn_wbb_standings`](../reference/index.qmd) | Conference standings + records | ESPN |
| [`espn_wbb_conferences`](../reference/index.qmd) | Conference groups + group ids | ESPN |
| [`espn_wbb_rankings`](../reference/index.qmd) | ⭐ AP / Coaches poll rankings | ESPN |
| [`espn_wbb_leaders`](../reference/index.qmd) | ⭐ League statistical leaders | ESPN |
| [`espn_wbb_injuries`](../reference/index.qmd) | ⭐ Active injury report | ESPN |
| [`espn_wbb_season_powerindex`](../reference/index.qmd) | ⭐ **BPI** ratings, one row per team | ESPN |
| [`espn_wbb_season_powerindex_leaders`](../reference/index.qmd) | ⭐ BPI / SOS / SOR category leaders | ESPN |
| [`espn_wbb_game_predictor`](../reference/index.qmd) | ⭐ BPI matchup projection for a game | ESPN |
| [`espn_wbb_game_probabilities`](../reference/index.qmd) | ⭐ Play-by-play win-probability curve | ESPN |
| [`espn_wbb_calendar`](../reference/index.qmd) | Valid game dates for a season | ESPN |
| [`load_wbb_schedule`](../reference/index.qmd) | Season-long schedule (parquet release) | release |
| [`load_wbb_pbp`](../reference/index.qmd) | Season-long play-by-play (parquet, back to 2002) | release |
| [`load_wbb_team_boxscore`](../reference/index.qmd) | Season-long team boxscores (parquet) | release |
| [`load_wbb_player_boxscore`](../reference/index.qmd) | Season-long player boxscores (parquet) | release |
...
[`espn_wbb_teams`](../reference/index.qmd) returns one wide row per Division-I program. The `team_id` here is the key you'll feed to roster, stats, and leader endpoints. (NCAA team frames carry no conference column — that comes from `espn_wbb_standings()` / `espn_wbb_conferences()` below.)
...
[`espn_wbb_team_roster`](../reference/index.qmd) takes a `team_id` and `season` and returns one row per player. Here's the 2024-25 **UConn Huskies** (`team_id=2509`) — the eventual national champions, led by Paige Bueckers.
...
| Function | Best for |
|---|---|
| [`espn_wbb_schedule`](../reference/index.qmd) | a clean game list for a date or `'YYYYMMDD-YYYYMMDD'` range |
| [`espn_wbb_scoreboard`](../reference/index.qmd) | ⭐ a richer live/final scoreboard (status, venue, scores) |
...
[`espn_wbb_pbp`](../reference/index.qmd) returns a **dict** of game components (`plays`, `boxscore`, `header`, `winprobability`, …). The `plays` value is a list of dicts — build a frame with `pl.DataFrame(pbp['plays'], infer_schema_length=None)`. Columns use ESPN dot-notation (`period.number`, `clock.displayValue`, `type.text`, `scoringPlay`).
...
| Function | Gives you |
|---|---|
| [`espn_wbb_rankings`](../reference/index.qmd) | the current AP / Coaches poll |
| [`espn_wbb_leaders`](../reference/index.qmd) | league statistical leaders (PPG, RPG, APG, …) |
| [`espn_wbb_injuries`](../reference/index.qmd) | the active injury report |
...
ESPN's **BPI** is a forward-looking team-strength rating — expected point margin per 70 possessions against an average opponent on a neutral floor. [`espn_wbb_season_powerindex`](../reference/index.qmd) returns one row per ranked team, with a nested `stats` list (BPI, BPI rank, SOS, SOR, …). Let's unnest it into a clean BPI leaderboard for 2024-25.
...
And [`espn_wbb_season_powerindex_leaders`](../reference/index.qmd) lists the category leaders — who tops BPI, strength-of-schedule, strength-of-record, and more.
...
[`espn_wbb_standings`](../reference/index.qmd) returns one wide row per team — records, win %, points for/against, **and** conference membership. [`espn_wbb_conferences`](../reference/index.qmd) lists the conference groups with their `group_id`s (handy for filtering).
...
[`espn_wbb_game_probabilities`](../reference/index.qmd) returns ESPN's play-by-play win-probability snapshots for a game. Let's watch how UConn's win odds evolved through the 2025 title game (event `401746075`).
...
[`espn_wbb_game_predictor`](../reference/index.qmd) gives ESPN's BPI-based projection for a single game — matchup quality, projected game score, and each side's predicted point total. Here's the championship preview.
...
[`espn_wbb_team_schedule`](../reference/index.qmd) returns one program's complete season — every game with its date, matchup short name, and season type. Here's UConn's 2024-25 road to the title (`team_id=2509`).
...
- **Premium analytics**: [`espn_wbb_season_powerindex`](../reference/index.qmd), [`espn_wbb_game_probabilities`](../reference/index.qmd), and [`espn_wbb_rankings`](../reference/index.qmd) are the deep cuts.
- **Full reference**: the WBB pages — [core](../reference/index.qmd), [site](../reference/index.qmd), [web](../reference/index.qmd), [additional](../reference/index.qmd), and [loaders](../reference/index.qmd).
- `dir(sdv.wbb)` shows the full 100+ endpoint surface (player gamelogs, splits, depth charts, transactions, recruits, and more).
- Men's side? See the parallel [`06_mbb_intro.ipynb`](07-mbb-intro.qmd).
