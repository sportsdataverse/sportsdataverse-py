---
title: "PWHL tutorial"
description: "PWHL walkthrough — the canonical sportsdataverse surface, ported from the Docusaurus tutorials. Code cells show the calls; run them in your own session."
jupyter: python3
---
...
|---|---|---|
| [`load_pwhl_schedule`](../reference/index.qmd) | Games + results, one row per game | 📦 loader |
| [`load_pwhl_rosters`](../reference/index.qmd) | One row per player per team (skaters + goalies) | 📦 loader |
| [`load_pwhl_skater_box`](../reference/index.qmd) | Skater boxscore, one row per player per game | 📦 loader |
| [`load_pwhl_goalie_box`](../reference/index.qmd) | Goalie boxscore (saves, shots against, GAA inputs) | 📦 loader |
| [`load_pwhl_team_box`](../reference/index.qmd) | Team boxscore (shots, PP, faceoffs) | 📦 loader |
| [`load_pwhl_pbp`](../reference/index.qmd) | Event-level play-by-play (wide, with coordinates) | 📦 loader |
| [`load_pwhl_scoring_summary`](../reference/index.qmd) | Tidy goal log (scorer + assists + situation flags) | 📦 loader |
| [`load_pwhl_penalty_summary`](../reference/index.qmd) | Tidy penalty log (infraction, minutes, who took it) | 📦 loader |
| [`load_pwhl_shots_by_period`](../reference/index.qmd) | Per-period shot & goal totals per game | 📦 loader |
| [`load_pwhl_three_stars`](../reference/index.qmd) | Post-game three-star selections | 📦 loader |
| [`pwhl_schedule`](../reference/index.qmd) | Live schedule, one row per game | 🛰️ live |
| [`pwhl_standings`](../reference/index.qmd) | Live standings, one row per team | 🛰️ live |
| [`pwhl_teams`](../reference/index.qmd) | Teams in a season (grab `team_id`s) | 🛰️ live |
| [`pwhl_team_roster`](../reference/index.qmd) | A team's roster | 🛰️ live |
| [`pwhl_leaders`](../reference/index.qmd) | Statistical leaders | 🛰️ live |
| [`pwhl_stats`](../reference/index.qmd) | Aggregate skater/goalie stats | 🛰️ live |
| [`pwhl_player_search`](../reference/index.qmd) | Find a player_id by name | 🛰️ live |
| [`pwhl_player_stats`](../reference/index.qmd) | A player's season-by-season stat lines | 🛰️ live |
| [`pwhl_pbp`](../reference/index.qmd) | Enriched single-game play-by-play | 🛰️ live |
| [`pwhl_game_corsi`](../reference/index.qmd) | On-ice Corsi / Fenwick per player | 🛰️ live |
| [`pwhl_player_toi`](../reference/index.qmd) | Time-on-ice per player | 🛰️ live |
| [`pwhl_game_shifts`](../reference/index.qmd) | Raw shift stints | 🛰️ live |
| [`most_recent_pwhl_season`](../reference/index.qmd) · [`pwhl_season_id`](../reference/index.qmd) | Season helpers | 🛰️ live |
...
[`load_pwhl_schedule`](../reference/index.qmd) returns one row per game with the result and a set of flag/URL columns pointing at the per-game feeds. Pass `seasons=[2024]` (a list — you can stack multiple seasons). ⚠️ Heads up: `home_score`/`away_score` come back as **strings**, so cast them before doing arithmetic.
...
[`load_pwhl_rosters`](../reference/index.qmd) gives one row per player per team, split into skaters and goalies via the `player_type` column.
...
|---|---|
| [`load_pwhl_team_box`](../reference/index.qmd) | team per game |
| [`load_pwhl_skater_box`](../reference/index.qmd) | skater per game |
| [`load_pwhl_goalie_box`](../reference/index.qmd) | goalie per game |
...
[`load_pwhl_pbp`](../reference/index.qmd) returns a wide event log. The `event` column tags each row as `faceoff`, `shot`, `goal`, or `penalty` — and there are several coordinate systems (`x_coord`/`y_coord` plus rink-normalized `*_fixed` / `*_right` variants) for drawing rink plots.
...
[`load_pwhl_penalty_summary`](../reference/index.qmd) is a tidy per-infraction log. Two quick cuts: the most common infractions league-wide, and the players spending the most time in the box.
...
Two compact joins-on-themselves. First, who collected the most **first-star** nods ([`load_pwhl_three_stars`](../reference/index.qmd)). Then a **head-to-head** series view from the schedule — swap in any two clubs.
...
A classic two-step lookup off the live feed: [`pwhl_player_search`](../reference/index.qmd) resolves a name to a `player_id`, then [`pwhl_player_stats`](../reference/index.qmd) returns her season-by-season stat lines. Both are `safe()`-wrapped for offseason resilience.
...
The full live tour. List teams with [`pwhl_teams`](../reference/index.qmd), grab a `team_id`, pull the roster with [`pwhl_team_roster`](../reference/index.qmd), take a `game_id` from the loader schedule, then fetch enriched events with [`pwhl_pbp`](../reference/index.qmd) and shot-attempt share with [`pwhl_game_corsi`](../reference/index.qmd) — all from the same feed. Everything is `safe()`-wrapped, so offline this prints a friendly note instead of raising.
...
Straight off the HockeyTech feed: [`pwhl_standings`](../reference/index.qmd) for the live table and [`pwhl_leaders`](../reference/index.qmd) for the statistical leaderboard. Both take a `season` end-year. We keep them `safe()`-wrapped because live endpoints are seasonal.
...
| Function | Metric |
|---|---|
| [`pwhl_game_corsi`](../reference/index.qmd) | Corsi / Fenwick shot-attempt share, with per-60 rates |
| [`pwhl_player_toi`](../reference/index.qmd) | summed time-on-ice + shift counts per player |
| [`pwhl_game_shifts`](../reference/index.qmd) | raw shift stints (who's on the ice, when) |
...
[`load_pwhl_scoring_summary`](../reference/index.qmd) is a clean per-goal log — scorer plus up to two assists, with situation flags like power play, short handed, and game-winning. And because every loader takes `return_as_pandas=True`, dropping into the pandas world is one keyword away.
...
- Full reference: the **PWHL → [Loaders](../reference/index.qmd)** and **[Additional functions](../reference/index.qmd)** pages in the sidebar.
