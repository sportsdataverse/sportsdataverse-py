---
title: "WNBA tutorial"
description: "WNBA walkthrough — the canonical sportsdataverse surface, ported from the Docusaurus tutorials. Code cells show the calls; run them in your own session."
jupyter: python3
---
...
|---|---|---|
| [`espn_wnba_teams`](../reference/index.qmd) | One row per franchise (grab `team_id`s) | ⭐ ESPN |
| [`espn_wnba_team_roster`](../reference/index.qmd) | A team's active roster for a season | ⭐ ESPN |
| [`espn_wnba_schedule`](../reference/index.qmd) | Games + results for a date or date range | ⭐ ESPN |
| [`espn_wnba_pbp`](../reference/index.qmd) | Event-level play-by-play for one game | ⭐ ESPN |
| [`espn_wnba_player_stats`](../reference/index.qmd) | A player's season stat line (wide) | ⭐ ESPN |
| [`espn_wnba_team_stats`](../reference/index.qmd) | A team's season stats (Averages/Totals/Misc) | ⭐ ESPN |
| [`espn_wnba_standings`](../reference/index.qmd) | League standings, one row per team | ⭐ ESPN |
| [`espn_wnba_draft`](../reference/index.qmd) | Every draft pick for a season | ⭐ ESPN |
| [`espn_wnba_game_officials`](../reference/index.qmd) | The refs who worked a game | ⭐ ESPN |
| [`load_wnba_schedule`](../reference/index.qmd) | Whole-season schedule (parquet release) | 📦 loader |
| [`load_wnba_player_boxscore`](../reference/index.qmd) | Whole-season player box scores | 📦 loader |
| [`load_wnba_team_boxscore`](../reference/index.qmd) | Whole-season team box scores | 📦 loader |
| [`load_wnba_player_season_stats`](../reference/index.qmd) | Season-aggregated player stats | 📦 loader |
| [`load_wnba_pbp`](../reference/index.qmd) | Whole-season play-by-play | 📦 loader |
| [`load_wnba_shots`](../reference/index.qmd) | Shot-location data | 📦 loader |
| [`load_wnba_standings`](../reference/index.qmd) | Whole-season standings (long) | 📦 loader |
| [`load_wnba_rosters`](../reference/index.qmd) | Whole-season rosters | 📦 loader |
| [`load_wnba_draft`](../reference/index.qmd) | Whole-season draft picks | 📦 loader |
| [`most_recent_wnba_season`](../reference/index.qmd) | The latest season year | 🛠️ helper |
...
[`espn_wnba_teams`](../reference/index.qmd) returns one row per franchise. The `team_id`, location, name and abbreviation are the keys you'll reuse to fetch rosters, schedules and stats.
...
[`espn_wnba_team_roster`](../reference/index.qmd) lists active players for one team in a season. The back-to-back champion Aces are `team_id=17`. Player columns are unprefixed (`athlete_id`, `full_name`, `jersey`, `position_abbreviation`).
...
[`espn_wnba_schedule`](../reference/index.qmd) takes `dates=YYYYMMDD` for a single day, or a `'YYYYMMDD-YYYYMMDD'` string for a range. Team-name columns are `home_display_name` / `away_display_name`, and `home_score` / `away_score` come back as **strings** — cast before doing arithmetic.
...
[`espn_wnba_pbp`](../reference/index.qmd) returns a **dict** of component pieces (`plays`, `boxscore`, `header`, `winprobability`, …). The `plays` entry is a list of raw ESPN dicts; build a frame with `pl.DataFrame(..., infer_schema_length=None)`. Its columns use raw **dot-notation** (`period.number`, `clock.displayValue`, `scoringPlay`, `type.text`).
...
[`espn_wnba_player_stats`](../reference/index.qmd) returns a single **wide** row covering ESPN's `general` / `offensive` / `defensive` stat groups (averages and totals). The 2024 Rookie of the Year, Caitlin Clark, is `athlete_id=4433403`. Pass `total=True` for season totals instead of per-game averages.
...
[`espn_wnba_team_stats`](../reference/index.qmd) returns a **dict** keyed by category — `{'Averages', 'Totals', 'Misc'}`. Each value is a long frame of `stat_name` / `display_value` rows, so index into the dict rather than calling `.head()` on the return directly.
...
[`espn_wnba_standings`](../reference/index.qmd) gives one row per team with wins, losses, win percentage and point differential. Sort by win percentage to get the playoff picture.
...
[`espn_wnba_draft`](../reference/index.qmd) lists every pick for a season. The 2024 draft headlined with Caitlin Clark going first overall to the Indiana Fever.
...
[`load_wnba_player_boxscore`](../reference/index.qmd) reads a whole season's player box scores from a parquet release (no per-game API calls). Drop did-not-play rows, then aggregate points and assists per player with polars. Loaders are reliable, so this one runs bare.
...
[`espn_wnba_game_officials`](../reference/index.qmd) returns the referees assigned to a game — handy for officiating studies. Pair a `game_id` from the schedule with this call.
...
[`load_wnba_team_boxscore`](../reference/index.qmd) carries each team's score **and** its opponent's score per game. Average points for minus points against gives a quick-and-dirty net rating — the single best one-number summary of who's good. We require 20+ games to drop the All-Star exhibition noise.
...
[`load_wnba_shots`](../reference/index.qmd) is event-level shot data with a `score_value` (the point value of the attempt). Tally made vs. attempted threes per team to see who lives behind the arc — and who actually makes them.
...
[`load_wnba_rosters`](../reference/index.qmd) hands you every team's full roster. Pivot guards / forwards / centers per team to see how each front office balances its lineup — a clean join-free `pivot`.
...
Don't want to roll up box scores yourself? [`load_wnba_player_season_stats`](../reference/index.qmd) ships ESPN's own season aggregates in **long** format (`category` / `stat_name` / `value`). Filter to the `averages` category and the `avgPoints` stat for an instant scoring leaderboard — a great cross-check against Recipe 3.
...
| Loader | Whole-season… |
|---|---|
| [`load_wnba_schedule`](../reference/index.qmd) | schedule + results |
| [`load_wnba_player_boxscore`](../reference/index.qmd) | player box scores |
| [`load_wnba_team_boxscore`](../reference/index.qmd) | team box scores |
| [`load_wnba_player_season_stats`](../reference/index.qmd) | season-aggregated player stats |
| [`load_wnba_pbp`](../reference/index.qmd) | play-by-play |
| [`load_wnba_shots`](../reference/index.qmd) | shot locations |
...
- Full reference: the **WNBA** section in the sidebar — [ESPN extras](../reference/index.qmd), [site API](../reference/index.qmd), [core API](../reference/index.qmd) and [loaders](../reference/index.qmd).
