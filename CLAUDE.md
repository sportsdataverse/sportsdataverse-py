...
### PFF — Developer API (`api.pff.com`, current) vs Premium (`premium.pff.com`, LEGACY)

- **Current: `pff_api_*`** (`sportsdataverse/nfl/pff_api.py`, GENERATED by
  `tools/codegen/gen_pff_api.py` from PFF's OWN OpenAPI spec vendored in sdv-internal-refs
  `pff/developer/pff-developer.openapi.json` + its live captures). One wrapper per PFF
  `operationId` (`pff_api_team_stats` == `restish pff team-stats`). Runtime
  `nfl/pff_api_runtime.py`: bearer key from `api_key=` > an `Authorization` in `headers=` >
  `SDV_PY_PFF_API_KEY` > `PFF_API_KEY`; 400/422 -> `ValueError`, 404 -> `NoDataError`,
  401/403/429/5xx or a non-object 200 -> `AssetFetchError` (403 = entitlement, never retried);
  `cache_ttl=0` always (the response cache keys ignore Authorization). A `restricted` list (columns
  withheld by entitlement) -> `UserWarning` + partial body; `strict=True` / `SDV_PY_PFF_STRICT=1`
  raises instead — pipelines must use strict.
- **`/v1` query keys are the spec's exact names — snake_case.** api.pff.com SILENTLY IGNORES
  camelCase `franchiseId`/`gameId` (returns the whole leaderboard); `test_gen_pff_api.py` locks
  it. `/v2/{league}` uses camelCase query keys (`weekGroup`, `weekIds`, `weekTo`) and `league` is a
  PATH param (a flat-API `{league}` token needed the `_sub_slugs` empty-slug fix in generate.py).
- `/v2` bodies parse with `parse_pff_v2_table` (types from the body's own `columns`; second table
  `teamTotals` on rushing-direction). The `restricted` block beside a `/v1` envelope is metadata —
  `parse_pff_report`/`parse_pff_player_detail` skip it.
- **LEGACY: `pff_*` / `pff_<league>_*`** (`pff_core.py` from `gen_pff.py`, cookie-auth
  `pff_runtime.py`). Every summary starts `LEGACY (...)`; keep them working, don't extend them.
- Public fixtures for `pff_api` are PFF's OWN published spec examples
  (`tests/fixtures/pff_api/`), never our captures. Live tests: `SDV_PY_PFF_LIVE=1` + a key.
- Regenerate: `SDV_INTERNAL_REFS_REPO=<path> uv run python tools/codegen/gen_pff_api.py` then
  `generate.py`. Budget: 100 reads/min per ACCOUNT, shared with the pff-*-raw scrapers.
