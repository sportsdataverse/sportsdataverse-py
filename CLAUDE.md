...
- The Docusaurus site lives under `docs/` — don't put hand-authored markdown
  there. The `docs/docs/<sport>/index.md` pages are regenerated from the
  Sphinx markdown build via `bash create_docs.sh`.
...
## Sphinx + docs build toolchain

- Source: `Sphinx-docs/conf.py` + auto-generated `*.rst` files from
  `sphinx-apidoc`. Build target: markdown via `sphinx-markdown-builder`.
- Regen: `bash create_docs.sh` (runs apidoc, builds markdown, copies into
  `docs/docs/<sport>/index.md` and `docs/src/pages/CHANGELOG.md`).
- The build is **clean under `sphinx-build -W`** (warnings-as-errors).
  Don't introduce docstrings that re-break it. Two specific gotchas:
  1. New module docstrings must use Google-style sections (`Args:`,
     `Returns:`, `Raises:`, `Example:`) — `sphinx.ext.napoleon` is wired
     up and parses those correctly. Free-form `Caveats:` /
     `<Heading>:` paragraph headers followed immediately by a bullet
     list need a **blank line** between the heading and the first bullet,
     and `*` markers (not `-`), or docutils will flag a definition list
     parse error.
  2. `conf.py` ships a no-op `visit_abbreviation` shim patched onto
     `sphinx_markdown_builder.MarkdownTranslator`. Sphinx 9 emits
     `<abbreviation>` nodes for the keyword-only `*` separator in
     rendered function signatures and the builder (0.6.10) has no
     visitor for that node type. Do NOT remove the patch unless you've
     upgraded `sphinx-markdown-builder` to a release that handles
     abbreviation natively.
